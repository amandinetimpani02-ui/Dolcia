...
export default async function handler(req, res) {
  ...
  // Aucun résultat ne peut contourner l'éligibilité géographique parce qu'il
  // se trouve après une limite arbitraire dans le classement.
  ...
  for (const item of eligibleCandidates) {
    ...
    const sourceType = /ticketmaster|billetterie/i.test(item.source || '') ? 'ticketing_platform' : 'tourism_office';
    // Une source officielle prouve l'existence du rendez-vous, jamais sa rareté ni sa singularité.
    // Ces deux derniers signaux doivent être fournis séparément et rester vérifiables.
    const proofEvidence = item.proofEvidence || (item.official && item.date ? {
      verified: true,
      source: item.source || 'Source officielle',
      sourceType,
      checkedAt: item.retrievedAt || new Date().toISOString()
    } : null);
    const result = await classifyCandidate({
      ...
      singularityEvidence: item.singularityEvidence || null,
      capacityRelevant: item.capacityRelevant === true,
      capacityEvidence: item.capacityEvidence || null,
      explicitRegionalScenario: item.explicitRegionalScenario === true || safeContext.explicitRegionalScenario === true,
    ...
    classified.push({ ...item, experienceKind: item.experienceKind || item.category, qualityScore: Math.round((item.rating || 0) * 20), result, matchesLocality });
  }
  const eligibility = new Map(applyAlternativeCheck(classified).map(item => [item.id, { ...item.result, destinationLocalityMatch: item.matchesLocality }]));
  const enriched = ranked.map(item => eligibility.has(item.id) ? { ...item, geoEligibility: eligibility.get(item.id) } : { ...item, geoEligibility: { status: 'outside', premium_eligible: false, blocking_reasons: ['NOT_CLASSIFIED'] } })
