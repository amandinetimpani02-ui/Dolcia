...
const DESTINATION_TIME_ZONE = 'Europe/Paris';
const WEEKDAY_INDEX = { Sun: 0, Mon: 1, Tue: 2, Wed: 3, Thu: 4, Fri: 5, Sat: 6 };
// Le process serveur (Vercel comme ce container) tourne en UTC : lire arrival.getDay()/getHours()
// directement décale silencieusement l'heure réelle de la destination (jusqu'à 2h en été).
// On projette systématiquement l'instant dans le fuseau de la destination avant de comparer aux horaires.
function localMoment(date, timeZone = DESTINATION_TIME_ZONE) {
  ...
  return { day: WEEKDAY_INDEX[parts.weekday], hour, minute: Number(parts.minute) };
}
function openForArrival(periods, arrival, minimumMinutes = 60) {
  ...
  const moment = localMoment(arrival);
  const target = moment.day * 1440 + moment.hour * 60 + moment.minute;
...
export function applyAlternativeCheck(items = []) {
  ...
  return items.map(item => {
    ...
    const betterRegional = !equivalent && items.some(other =>
      ...
      other.experienceKind === item.experienceKind &&
      (other.result.travel_minutes ?? Infinity) + 10 < (item.result.travel_minutes ?? Infinity) &&
      (other.qualityScore || 0) >= (item.qualityScore || 0) - 6
    );
