export default async function handler(req, res) {
  ...
  const LOC = `${lat},${lng}`;
  const radiusSafe = Math.min(Number(radius) || 5000, 50000);

  try {
    let url;
    if (mode === "text" && keyword) {
      url = `${BASE}/textsearch/json?query=${encodeURIComponent(keyword)}&location=${LOC}&radius=${radiusSafe}&language=fr&key=${GK}`;
    } else if (type) {
      url = `${BASE}/nearbysearch/json?location=${LOC}&radius=${radiusSafe}&type=${encodeURIComponent(type)}&language=fr&key=${GK}`;
