...
export default async function handler(req, res) {
  ...
  if (!base || !key) return res.status(200).json({ venues: [], configured: false });
  ...
  try {
    ...
    return res.status(200).json({ venues, configured: true, onlyConfirmed: true });
  } catch (_error) {
    return res.status(200).json({ venues: [], configured: true, unavailable: true });
  }
}
