...
function deepGeoDT(value) { if (!value || typeof value !== 'object') return null; const lat = Number(value.latitude ?? value.lat), lng = Number(value.longitude ?? value.lng ?? value.lon); if (Number.isFinite(lat) && Number.isFinite(lng)) return { lat, lng }; for (const child of Object.values(value)) { const found = deepGeoDT(child); if (found) return found; } return null; }
function regionalGemKind(item, title) {
  const text = `${title || ''} ${Array.isArray(item.type) ? item.type.join(' ') : item.type || ''}`.toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '');
  const rules = [[/atelier|stage|enfant|tout.petit/,'Ateliers enfants'],[/artisan|savoir.faire|poterie|ceramique|verrier|tissage/,'Artisanat & savoir-faire'],[/fete de village|fete locale|kermesse|bal populaire|ducasse/,'Fêtes de village'],[/feu d.artifice/,'Feux d’artifice'],[/braderie|vide.grenier|brocante|marche aux puces/,'Braderies & vide-greniers'],[/ferme|animal|cueillette|nature/,'Ferme & nature'],[/patrimoine|visite|chateau|moulin|eglise|phare/,'Patrimoine']];
  return rules.find(([pattern]) => pattern.test(text))?.[1] || null;
}
function normalizeDT(item) { const title = localizedDT(item.label); if (!title) return null; const date = deepDateDT(item); const address = item.isLocatedAt?.address || {}; const gemKind = regionalGemKind(item, title), geo = deepGeoDT(item.isLocatedAt || item); return { id: `datatourisme-${item.uuid}`, title, date, location: localizedDT(address.hasAddressCity?.label) || '', address: [address.hasAddressNumber, address.hasAddressStreet?.label || address.hasAddressStreet, address.hasPostalCode].filter(Boolean).join(' '), lat: geo?.lat ?? null, lng: geo?.lng ?? null, type: Array.isArray(item.type) ? item.type.join(' ') : item.type, image: deepUrlDT(item.hasMainRepresentation), registrationUrl: item.uri, source: 'DATAtourisme · Offices de tourisme de France', official: true, regionalGem: Boolean(gemKind), regionalGemKind: gemKind, retrievalScope: gemKind ? 'signature' : 'local' }; }
async function handleDatatourisme(req, res) {
  ...
  const lat = Number(req.query.lat), lng = Number(req.query.lng), radius = Math.min(Math.max(Number(req.query.radius || 30), 45), 60);
