...
//   POST ?service=partner-submit   → Écriture réelle candidature/établissement/publication partenaire (Supabase)
//   POST ?service=create-checkout  → Session de paiement Stripe pour les paliers Pro
...
// ── Écriture réelle des données partenaire (candidature/établissement/publication) ──
...
const PARTNER_TIERS = {
  visibilite: { amount: 2900, label: 'Dolcia Pro — Visibilité (mensuel)', recurring: true },
...
async function handleCreateCheckout(req, res) {
  ...
  if (!secretKey) return res.status(200).json({ ok: false, configured: false, error: 'Stripe non configuré (STRIPE_SECRET_KEY manquante)' });
  const { tier, organizationId, successUrl, cancelUrl } = req.body || {};
  const plan = PARTNER_TIERS[tier];
  if (!plan) return res.status(400).json({ ok: false, error: 'Palier inconnu (visibilite, premium, flash)' });
  const params = new URLSearchParams();
  params.set('mode', plan.recurring ? 'subscription' : 'payment');
  params.set('success_url', String(successUrl || '').slice(0, 500) || 'https://dolcia.vercel.app/pro.html?checkout=success');
  params.set('cancel_url', String(cancelUrl || '').slice(0, 500) || 'https://dolcia.vercel.app/pro.html?checkout=cancelled');
  ...
  if (organizationId) params.set('client_reference_id', String(organizationId).slice(0, 200));
  try {
    const r = await fetch('https://api.stripe.com/v1/checkout/sessions', { method: 'POST', headers: { Authorization: `Bearer ${secretKey}`, 'Content-Type': 'application/x-www-form-urlencoded' }, body: params.toString() });
    const data = await r.json();
...
async function handlePartnerSubmit(req, res) {
  ...
  const spec = PARTNER_TABLES[action];
  if (!spec) return res.status(400).json({ ok: false, error: 'action inconnue (organization, establishment, event, program)' });
  const missing = spec.required.filter(field => payload?.[field] === undefined || payload?.[field] === null || payload?.[field] === '');
  if (missing.length) return res.status(400).json({ ok: false, error: `Champs manquants : ${missing.join(', ')}` });
  const row = {}; for (const field of spec.allowed) if (payload[field] !== undefined) row[field] = payload[field];
  try {
    const r = await fetch(`${url}/rest/v1/${spec.table}`, { method: 'POST', headers: { apikey: key, Authorization: `Bearer ${key}`, 'Content-Type': 'application/json', Prefer: 'return=representation' }, body: JSON.stringify(row) });
    if (!r.ok) { const detail = await r.text().catch(() => ''); return res.status(200).json({ ok: false, error: `Supabase a refusé l'écriture (${r.status})`, detail: detail.slice(0, 300) }); }
    const created = await r.json();
    return res.status(200).json({ ok: true, id: created?.[0]?.id || null });
  } catch (error) { return res.status(200).json({ ok: false, error: error.message }); }
