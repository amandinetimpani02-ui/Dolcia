...
const state = {
  ...
  location:{name:'Le Touquet-Paris-Plage',lat:50.5214,lng:1.5912}, answers:{vibes:[]}, items:[], allItems:[], weather:null, radius:12000,
...
async function compose(){
  ...
  try{
    ...
    const placeQueries=queries.slice(0,12).map(q=>`/api/places?lat=${state.location.lat}&lng=${state.location.lng}&radius=${state.radius}&mode=text&keyword=${encodeURIComponent(q+' '+state.location.name)}`);
    ...
    state.items=state.allItems.slice(0,24);
...
function queriesForVibes(){
  const selectedMap={food:['restaurant','brunch salon de thé','gastronomie dégustation'],outside:['plage balade nature','sports nautiques'],culture:['musée exposition patrimoine','spectacle théâtre cinéma'],slow:['spa thalasso bien-être'],active:['parc attractions loisirs','escape game bowling karting'],night:['concert bar soirée']};
  const broad=['restaurants et cafés','parcs jardins et nature','animations et activités famille enfants','parcs de loisirs attractions','musées expositions patrimoine','concerts spectacles théâtre cinéma','sports loisirs plein air nautique','escape game bowling karting','spa thalasso bien-être','marchés brocantes ateliers locaux'];
...
function renderResults(){
  ...
  app.innerHTML=shell(`<section class="results-hero"><div class="results-photo" style="background-image:url('${IMAGES.hero}')"></div><div class="results-copy"><span class="kicker">Composé pour vous</span><h1>Votre moment<br>prend forme.</h1><p>${state.weather?.main?.temp?`${Math.round(state.weather.main.temp)}°, `:''}${esc(state.location.name)}. Une sélection réelle et variée, pensée selon votre rythme.</p><div class="tags">${criteria.map(x=>`<span class="tag">${esc(x)}</span>`).join('')}</div><button class="surprise-hero" onclick="surprise()"><span>Je veux une autre idée</span><strong>Surprends-moi</strong><b>↻</b></button></div></section><section class="program">${weatherPlan()}<div class="program-head"><div><span class="kicker">Votre sélection</span><h2>${state.items.length?'À vous de choisir':'Une autre piste ?'}</h2></div><div class="program-actions"><button class="secondary" onclick="surprise()">Nouvelle sélection</button><button class="secondary" onclick="startCompose()">Ajuster</button></div></div>${state.items.length?state.items.map(experience).join(''):emptyState()}${state.items.length?`<button class="add-all" onclick="addAll()"><span>Conserver ces idées</span><strong>Ajouter la sélection à mon agenda</strong><b>→</b></button>`:''}</section>`,'discover');
}
...
function surprise(){if(!state.allItems.length){state.radius=Math.min(state.radius*1.6,60000);showToast('Dolcia élargit la recherche…');return compose()}const pool=[...state.allItems].sort(()=>Math.random()-.5);state.items=pool.slice(0,24);renderResults();showToast('Une nouvelle sélection, avec les mêmes envies')}
