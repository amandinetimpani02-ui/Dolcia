...
function expireTemporaryGroups(now=Date.now()){
  const expiredIds=new Set(state.temporaryGroups.filter(group=>new Date(group.expiresAt).getTime()<now).flatMap(group=>group.memberIds||[]));
  state.temporaryGroups=state.temporaryGroups.filter(group=>new Date(group.expiresAt).getTime()>=now);
  if(expiredIds.size){
    state.circleProfiles=state.circleProfiles.filter(person=>!expiredIds.has(person.id));
    state.groupParticipants=state.groupParticipants.filter(person=>!expiredIds.has(person.id));
  }
  localStorage.setItem('dolcia_temporary_groups_v1',JSON.stringify(state.temporaryGroups));
}
expireTemporaryGroups();
...
let animateAutoAdvanceTimer=null;
// Avancement mains libres pour les programmes structurés (sport) : la voix parle déjà seule à
// chaque étape, mais il fallait encore toucher l'écran pour passer à la suivante — impossible à
// faire pendant un effort physique. Le minuteur avance seul, calé sur les horaires du programme.
function parseStepSeconds(label){const match=/(\d{1,2}):(\d{2})/.exec(label||'');return match?(Number(match[1])*60+Number(match[2]))*60:0}
function scheduleAnimateAutoAdvance(session){
  window.clearTimeout(animateAutoAdvanceTimer);
  if(!session.item.phased||session.status!=='active')return;
  const steps=session.item.steps,current=parseStepSeconds(steps[session.index][0]);
  const next=session.index<steps.length-1?parseStepSeconds(steps[session.index+1][0]):current+45;
  const durationMs=Math.max(8,next-current)*1000;
  animateAutoAdvanceTimer=window.setTimeout(()=>{if(state.activeAnimate===session)completeDolciaAnimateStep()},durationMs);
}

const esc = value => String(value??'').replace(/[&<>"']/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[c]));

// Comportement commun à toutes les fenêtres modales — une seule mécanique pour toute l'application,
// jamais à répéter fenêtre par fenêtre. Échap ferme, le focus reste emprisonné dedans tant qu'elle
// est ouverte, il revient sur l'élément qui l'avait ouverte à la fermeture, le fond ne défile plus
// pendant ce temps, et le bouton Retour mobile ferme la fenêtre plutôt que de quitter l'application.
let modalOpenerStack=[];
let modalClosingViaPopstate=false;
let modalSuppressNextPopstate=false;
function focusableIn(container){return[...container.querySelectorAll('button,[href],input,select,textarea,[tabindex]:not([tabindex="-1"])')].filter(el=>!el.disabled&&el.offsetParent!==null)}
function topModal(){const modals=document.querySelectorAll('.modal');return modals[modals.length-1]||null}
// Le fond doit devenir réellement inerte (pas seulement visuellement bloqué) pendant qu'une
// fenêtre est ouverte — sinon un lecteur d'écran peut encore parcourir ce qui est derrière.
// Toute fenêtre qui n'est plus la plus haute de la pile devient inerte aussi, pas seulement le
// contenu principal de l'application.
function updateInertState(){
  const modals=[...document.querySelectorAll('.modal')],top=modals[modals.length-1];
  const appEl=document.querySelector('#app');
  if(appEl)appEl.inert=modals.length>0;
  modals.forEach(m=>{m.inert=m!==top});
}
// Le focus initial ne doit jamais tomber par défaut sur la croix de fermeture simplement parce
// qu'elle est le premier élément dans le HTML — [autofocus] d'abord, sinon le premier élément
// utile qui n'est pas le bouton fermer, et seulement en tout dernier recours la croix elle-même.
function pickInitialFocus(modal){
  const explicit=modal.querySelector('[autofocus]');if(explicit)return explicit;
  const useful=focusableIn(modal).filter(el=>!el.classList.contains('close'));
  return useful[0]||modal.querySelector('.close')||modal;
}
function enhanceModal(modal){
  ...
  modalOpenerStack.push(document.activeElement);
  document.body.style.overflow='hidden';
  updateInertState();
  pickInitialFocus(modal).focus?.();
  history.pushState({dolciaModal:true},'');
}
// Deux drapeaux distincts, jamais confondus : l'un neutralise le popstate que NOUS déclenchons
// nous-mêmes en consommant l'historique (fermeture normale, Échap ou ×) ; l'autre signale qu'un
// vrai geste retour est déjà en train de fermer la fenêtre, pour ne jamais consommer l'historique
// une deuxième fois pour la même action. C'est l'absence de cette distinction qui provoquait la
// fermeture en cascade de deux fenêtres imbriquées pour un seul geste.
function retireModal(){
  const opener=modalOpenerStack.pop();
  if(opener&&document.contains(opener))opener.focus?.();
  if(!document.querySelector('.modal'))document.body.style.overflow='';
  updateInertState();
  if(modalClosingViaPopstate){modalClosingViaPopstate=false;return}
  modalSuppressNextPopstate=true;
  history.back();
}
document.addEventListener('keydown',e=>{
  ...
  if(e.key==='Tab'){
    const focusables=focusableIn(modal);if(!focusables.length)return;
...
window.addEventListener('popstate',()=>{
  if(modalSuppressNextPopstate){modalSuppressNextPopstate=false;return}
  const modal=topModal();if(modal){modalClosingViaPopstate=true;modal.remove()}
});
new MutationObserver(mutations=>{
  for(const m of mutations){
    for(const node of m.addedNodes)if(node.nodeType===1&&node.classList?.contains('modal'))enhanceModal(node);
    for(const node of m.removedNodes)if(node.nodeType===1&&node.classList?.contains('modal'))retireModal();
  }
...
function setDVisualState(mood='idle',duration=0){
  const allowed=['idle','listening','thinking','speaking','delighted','calm','encouraging','dancing'];
...
function beginExplore(){if(!state.momentQualified)return openEclatDialogue(false,'explore');openExplorer()}
...
function dCoachNextQuestion(){
  ...
  return{
    ...
    choices:[['choose','Laisser D décider','Le meilleur accord, expliqué et modifiable'],['animate','D anime ce moment','Accueil, rythme, relances et conclusion guidés par D'],['adjust','Affiner une nuance','Rythme, distance, surprise ou météo'],['budget','Protéger le budget','Équilibrer sans appauvrir l’expérience']]
  };
}
...
function mountDCoachSuggestion(action){
  ...
  const labels={choose:['Composer maintenant','D prépare un programme modifiable'],animate:['Confier le moment à D','D accueille, cadence, encourage, adapte et conclut'],adjust:['Ajuster avec D','Rythme, distance ou intensité'],budget:['Ouvrir le budget vivant','Équilibrer sans niveler']};
...
// Une pépite locale déjà trouvée et vérifiée par le moteur géo (status 'extended') mérite d'être
// mentionnée spontanément en conversation — pas seulement visible si la personne clique le bon
// filtre. Jamais inventée : uniquement un item déjà présent dans state.allItems, déjà passé par
// toutes les vérifications (commune, rareté, budget de temps de trajet).
function currentPepiteHighlight(){
  const pepite=state.allItems.filter(item=>item.geoEligibility?.status==='extended').sort((a,b)=>(b.score||0)-(a.score||0))[0];
  return pepite?{name:pepite.name,why:pepite.date?`le ${new Date(pepite.date).toLocaleDateString('fr-FR',{day:'numeric',month:'long'})}`:'en ce moment'}:null;
}
function buildLiveContext(mode){
  ...
  return{
    ...
    programSize:context.programSize,agendaSize:context.agendaSize,pepite:currentPepiteHighlight()
  };
}
...
// Point d'entrée unique utilisé partout où D parle réellement (Anime, coach). La musique de fond
// baisse automatiquement pendant que cette fonction parle, et remonte doucement une fois finie —
// jamais les deux à plein volume en même temps, quel que soit le chemin de secours utilisé en
// dessous (flux, tamponné, ou repli navigateur).
function playPremiumVoice(text,onDone){
  const restoreTheme=duckDolciaThemeForSpeech();
  playPremiumVoiceInner(text,()=>{restoreTheme?.();onDone()});
}
function playPremiumVoiceInner(text,onDone){
  const mood=voiceMoodFromText(text);
...
// Deux previews comparables du comportement de la barre de navigation sur le Home, activables par
// ?homePreview=A ou ?homePreview=B — jamais actif par défaut, pour ne rien changer en production
// avant validation. Version A : la barre reste absente tout le temps qu'on est sur le Home.
// Version B : la barre reste absente sur le premier écran, puis apparaît une fois qu'on a défilé
// au-delà de l'affiche. Le moteur et les parcours restent strictement identiques dans les deux cas.
function homePreviewMode(){
  try{return new URLSearchParams(window.location.search).get('homePreview')}catch{return null}
}
function applyHomeNavPreview(){
  const mode=homePreviewMode();
  document.body.classList.remove('home-nav-hidden-always','home-nav-hidden-until-scroll');
  window.removeEventListener('scroll',homeNavScrollReveal);
  if(mode==='A')document.body.classList.add('home-nav-hidden-always');
  else if(mode==='B'){
    document.body.classList.add('home-nav-hidden-until-scroll');
    window.addEventListener('scroll',homeNavScrollReveal,{passive:true});
  }
}
function homeNavScrollReveal(){
  const poster=document.querySelector('#homePoster');
...
function openEclatDialogue(voice=false,intent='compose'){
  document.querySelector('#eclatDialogue')?.remove();
  state.eclatIntent=intent;
...
const ECLAT_LABELS={now:'Maintenant',tonight:'Ce soir',tomorrow:'Demain',custom:'À la date choisie',couple:'En amoureux',family:'En famille',friends:'Entre amis',colleagues:'Entre collègues',solo:'Pour moi',play:'Rire et bouger',breathe:'Prendre l’air',recharge:'Décrocher',taste:'Bien manger',create:'Découvrir',vibrate:'Vibrer ensemble',none:'Aucune contrainte',low_walk:'Peu de marche',shelter:'À l’abri',quiet:'Ambiance calme',accessible:'Accessible','0':'Gratuit confirmé','60':'60 €','150':'150 €','300':'300 €',flexible:'Expérience prioritaire'};
...
function renderEclatQuestion(){const target=document.querySelector('#eclatQuestion'),brief=state.eclatBrief;if(!target||!brief)return;const screens=[
  {q:'Quel jour venez-vous, et à quelle heure ?',sub:'La durée seule ne suffit jamais. Choisissez votre jour, votre heure de début et votre heure de fin pour révéler les événements et les bonnes idées réellement disponibles.',options:[['custom','Ouvrir mon agenda · jour, heures et durée'],['now','Maintenant · démarrer tout de suite pour 2 heures'],['tonight','Ce soir · de 18 h à 23 h'],['tomorrow','Demain · de 9 h à 18 h']]},
  ...
  {q:'Qu’est-ce qui pourrait gâcher le moment ?',sub:'Une seule contrainte suffit pour écarter les mauvaises idées avant qu’elles ne vous soient proposées.',options:[['none','Rien à signaler'],['low_walk','Éviter de trop marcher'],['shelter','Rester à l’abri'],['quiet','Éviter le bruit et la foule'],['accessible','Accessibilité indispensable']]},
  ...
  ];const screen=screens[brief.step];target.classList.remove('question-arrival');target.innerHTML=`<div class="dialogue-progress"><i style="--progress:${(brief.step+1)*20}%"></i><span>0${brief.step+1} / 05</span></div><h2>${screen.q}</h2><p>${screen.sub}</p><div class="dialogue-choices">${screen.options.map(([id,label])=>`<button onclick="answerEclat('${id}')"><span>${label}</span><b>→</b></button>`).join('')}</div><div class="dialogue-free"><input id="eclatFree" placeholder="${brief.step===4?'Ou indiquez votre montant total…':'Ou dites-le avec vos mots…'}" ${brief.step===4?'inputmode="decimal"':''} onkeydown="if(event.key==='Enter')answerEclatFree()"><button onclick="answerEclatFree()">Continuer</button><button class="voice" onclick="startEclatVoice()" aria-label="Parler à Dolcia">●</button></div>`;requestAnimationFrame(()=>target.classList.add('question-arrival'));updateEclatScene()}
function answerEclat(value){const brief=state.eclatBrief;if(!brief)return;if(brief.step===0){brief.answers.date=value;if(value==='custom')return renderEclatCustomDate();brief.answers.duration=value==='tonight'?'evening':value==='tomorrow'?'day':'2h'}if(brief.step===1){brief.answers.who=value;if(value==='solo'){selectSoloMoment();brief.step++;return renderEclatQuestion()}return renderEclatGroupQuestion()}if(brief.step===2){brief.answers.vibe=value;return advanceAfterVibe()}if(brief.step===3)brief.answers.constraint=value;if(brief.step===4)brief.answers.budget=value;if(brief.step<4){brief.step++;renderEclatQuestion()}else finishEclat()}
function renderEclatCustomDate(){
  const target=document.querySelector('#eclatQuestion');if(!target)return;
  const now=new Date(),today=iso(now),startTime=`${String(Math.max(8,now.getHours())).padStart(2,'0')}:00`,endTime=`${String(Math.min(23,Math.max(10,now.getHours()+2))).padStart(2,'0')}:00`,brief=state.eclatBrief;
  brief.calendarMonth=brief.calendarMonth||new Date(now.getFullYear(),now.getMonth(),1);brief.calendarStart=brief.calendarStart||today;brief.calendarEnd=brief.calendarEnd||today;
  target.innerHTML=`<span class="dialogue-step">Votre agenda de visite</span><h2>Quels jours serez-vous là ?</h2><p>Cliquez sur l’arrivée puis sur le départ. Dolcia révélera les rendez-vous uniques qui existent vraiment pendant votre présence.</p>${eclatCalendarMarkup()}<div class="eclat-custom-date exact-calendar"><label>Arrivée<input id="eclatStartDate" type="date" min="${today}" value="${brief.calendarStart}" onchange="syncEclatCalendarInputs()"></label><label>Heure d’arrivée<input id="eclatStartTime" type="time" value="${startTime}"></label><label>Départ<input id="eclatEndDate" type="date" min="${today}" value="${brief.calendarEnd}" onchange="syncEclatCalendarInputs()"></label><label>Heure de fin<input id="eclatEndTime" type="time" value="${endTime}"></label><label class="calendar-duration">Format<select id="eclatDuration"><option value="2h">Environ 2 heures</option><option value="morning">La matinée</option><option value="afternoon">L’après-midi</option><option value="afternoon_evening">L’après-midi + la soirée</option><option value="evening">La soirée</option><option value="day">La journée complète</option><option value="stay" ${brief.calendarStart!==brief.calendarEnd?'selected':''}>Plusieurs jours / séjour</option></select></label><aside><b>Dolcia surveille ces dates</b><span>Concerts, spectacles, grands matchs, phénomènes naturels et pépites locales ponctuelles seront croisés avec vos jours et vos heures.</span></aside><button onclick="answerEclatCustomDate()">Valider ces dates →</button></div>`
}
function eclatCalendarMarkup(){
  const brief=state.eclatBrief,month=brief.calendarMonth,y=month.getFullYear(),m=month.getMonth(),first=(new Date(y,m,1).getDay()+6)%7,total=new Date(y,m+1,0).getDate(),today=new Date();today.setHours(0,0,0,0);let cells='';
  for(let i=0;i<first;i++)cells+='<span class="empty"></span>';
  for(let day=1;day<=total;day++){const date=new Date(y,m,day),key=iso(date),disabled=date<today,selected=key===brief.calendarStart||key===brief.calendarEnd,inRange=brief.calendarStart&&brief.calendarEnd&&key>brief.calendarStart&&key<brief.calendarEnd;cells+=`<button ${disabled?'disabled':''} class="${selected?'selected ':''}${inRange?'in-range':''}" onclick="pickEclatCalendarDay('${key}')"><span>${day}</span>${selected?`<small>${key===brief.calendarStart?'Arrivée':'Départ'}</small>`:''}</button>`}
  const nights=brief.calendarStart&&brief.calendarEnd?Math.max(0,Math.round((new Date(`${brief.calendarEnd}T12:00:00`)-new Date(`${brief.calendarStart}T12:00:00`))/86400000)):0;
  return`<section class="eclat-visit-calendar"><header><button onclick="moveEclatCalendar(-1)">←</button><div><strong>${month.toLocaleDateString('fr-FR',{month:'long',year:'numeric'})}</strong><small>${nights?`${nights+1} jours · ${nights} nuit${nights>1?'s':''}`:'Choisissez votre arrivée puis votre départ'}</small></div><button onclick="moveEclatCalendar(1)">→</button></header><div class="weekdays">${['Lun','Mar','Mer','Jeu','Ven','Sam','Dim'].map(day=>`<span>${day}</span>`).join('')}</div><div class="days">${cells}</div></section>`
}
function moveEclatCalendar(delta){const brief=state.eclatBrief;brief.calendarMonth=new Date(brief.calendarMonth.getFullYear(),brief.calendarMonth.getMonth()+delta,1);renderEclatCustomDate()}
function pickEclatCalendarDay(key){const brief=state.eclatBrief;if(!brief.calendarStart||brief.calendarComplete){brief.calendarStart=key;brief.calendarEnd=key;brief.calendarComplete=false}else if(key<brief.calendarStart){brief.calendarEnd=brief.calendarStart;brief.calendarStart=key;brief.calendarComplete=true}else{brief.calendarEnd=key;brief.calendarComplete=true}renderEclatCustomDate()}
function syncEclatCalendarInputs(){const brief=state.eclatBrief,start=document.querySelector('#eclatStartDate')?.value,end=document.querySelector('#eclatEndDate')?.value;if(start)brief.calendarStart=start;if(end)brief.calendarEnd=end;brief.calendarComplete=true;renderEclatCustomDate()}
function answerEclatCustomDate(){
  const startDate=document.querySelector('#eclatStartDate')?.value,startTime=document.querySelector('#eclatStartTime')?.value,endDate=document.querySelector('#eclatEndDate')?.value,endTime=document.querySelector('#eclatEndTime')?.value,duration=document.querySelector('#eclatDuration')?.value;
...
function selectSoloMoment(){
  ...
  state.eclatBrief.answers.participantIds=['me'];
}
function eclatAvailableProfiles(){expireTemporaryGroups();return state.circleProfiles.filter(person=>person.id!=='me')}
...
function saveEclatQuickPerson(kind){
  const names=(document.querySelector('#quickPersonNames')?.value||'').split(',').map(name=>name.trim()).filter(Boolean);if(!names.length)return showToast('Indiquez au moins un prénom');
  const temporary=kind==='temporary',stamp=Date.now(),ids=[];
  names.forEach((name,index)=>{const person={id:`${temporary?'guest':'circle'}-${stamp}-${index}`,name,kind:temporary?'temporary':'relative',relationship:temporary?'Invité du séjour':document.querySelector('#quickRelation')?.value||'Proche'};state.circleProfiles.push(person);ids.push(person.id)});
  if(temporary){const end=state.eclatBrief?.answers.endDateObject||state.eclatBrief?.answers.dateObject||state.dateEnd.toISOString(),expires=new Date(end);expires.setHours(23,59,59,999);const code=Math.random().toString(36).slice(2,8).toUpperCase();state.temporaryGroups.push({id:`group-${stamp}`,name:document.querySelector('#quickGroupName')?.value.trim()||'Groupe temporaire',code,inviteUrl:`${location.origin}${location.pathname}?groupe=${code}`,memberIds:ids,expiresAt:expires.toISOString()})}
  const selected=new Set(state.eclatBrief.answers.participantIds||['me']);ids.forEach(id=>selected.add(id));state.eclatBrief.answers.participantIds=[...selected];save();document.querySelector('#eclatQuickPerson')?.remove();renderEclatGroupQuestion();showToast(temporary?'Groupe temporaire créé · code prêt à partager':'Proche ajouté au Cercle')
}
...
function finishEclat(){const a=state.eclatBrief.answers,intent=state.eclatIntent||'compose';if(a.date==='custom'&&a.dateObject){state.dateMode='custom';state.dateStart=new Date(a.dateObject);state.dateEnd=new Date(a.endDateObject||a.dateObject)}else setDate(a.date||'now');state.answers.who=a.who||state.answers.who||'solo';if(state.answers.who==='solo')selectSoloMoment();state.answers.vibes=[a.vibe||'breathe'];const constraintText={low_walk:'Éviter de trop marcher',shelter:'Rester à l’abri',quiet:'Éviter le bruit et la foule',accessible:'Accessibilité indispensable'}[a.constraint];if(constraintText)state.answers.momentSentence=[state.answers.momentSentence,constraintText].filter(Boolean).join(' · ');const amount=a.budget==='flexible'?null:Number(a.budget);state.budgetPlan.amount=Number.isFinite(amount)?amount:null;state.answers.budget=a.budget==='0'?'free':a.budget==='flexible'?'flexible':'custom';state.answers.duration=a.duration||(a.date==='tonight'?'evening':'2h');state.answers.stayRhythm=state.answers.duration==='stay'?(a.stayRhythm||inferStayRhythm()):null;state.momentQualified=true;localStorage.setItem('dolcia_moment_qualified_v1','1');localStorage.setItem('dolcia_moment_schema_v1',MOMENT_SCHEMA_VERSION);resetMomentCatalogFilters();save();document.querySelector('#eclatDialogue')?.remove();compose(intent==='explore')}
...
// Niveau 1 du Home : une seule phrase, toujours différente d'un jour à l'autre, jamais deux fois la
// même formule d'affilée. Choisie une fois par jour (stable pour la session, comme le reste du
// classement) à partir d'un vrai contexte quand il est connu — jamais une intuition sur l'humeur.
// Home v3 — une affiche, pas une fiche. Une seule image porte l'émotion ; le texte flotte dessus,
// jamais dans un cadre. La phrase (Niveau 1) et la proposition (Niveau 2) partagent la même image :
// deux moments de lecture, une seule scène, comme une vraie invitation plutôt qu'un dashboard.
// Les 20 séquences retenues après audit éditorial (content/home-selection-20.md) — vouvoiement
// uniquement, aucune affirmation non fondée, aucune pression. Chaque clé correspond à un signal
// déjà réellement disponible dans le code, jamais une donnée à inventer.
const HOME_SEQUENCES={
  ...
  // Dolcia ne doit pas attendre un événement exceptionnel pour créer une émotion — mais sans fait
  // vérifié (météo, horaire, agenda), elle ne doit jamais prétendre avoir observé quelque chose.
  // Ces lignes restent purement émotionnelles, jamais une observation déguisée en révélation.
  ordinary:[
    {signature:'Attendez…',beats:['Votre journée n’est pas encore écrite.'],reveal:'Je peux m’occuper de la suite.'},
    {signature:'Rien de prévu ?',beats:['Ce n’est pas un problème.'],reveal:'Laissez-moi vous guider.'},
    {signature:'Regardez.',beats:['Une journée qui commence sans plan', 'n’est pas une journée perdue.'],reveal:'Construisons-la ensemble.'},
    {signature:'Venez.',beats:['Il n’est jamais trop tôt pour bien commencer.'],reveal:'Je m’occupe du reste.'}
  ]
};
// Priorité de sélection : pépite prouvée et événement unique (les plus rares, donc les plus
// désirables — ARCHITECTURE.md §5) avant la météo, elle-même avant la composition du groupe connue.
// Jamais deux contextes en même temps, jamais une supposition sans signal réel derrière.
function pickHomeSequenceContext(moment){
  if(moment){
    if(moment.geoEligibility?.status==='extended')return'gem';
    if(moment.date&&moment.confirmedUnique)return'unique_event';
  }
  const weather=(state.weather?.weather?.[0]?.main||'').toLowerCase();
  if(moment&&/rain|drizzle|thunder/.test(weather))return'rain';
  if(moment&&(/wind|squall/.test(weather)||Number(state.weather?.wind?.speed)>10))return'wind';
  if(moment&&/clear/.test(weather))return'sun';
  const who=state.answers?.who;
  if(moment&&who==='family')return'family';
  if(moment&&who==='couple')return'couple';
  // Aucun grand moment aujourd'hui — ce n'est pas une raison de rester silencieux. Un mardi
  // ordinaire mérite aussi sa révélation, tant qu'elle reste honnête (jamais un lieu inventé).
  return'ordinary';
}
function homeCinemaAlreadyShownToday(){
  const today=new Date().toISOString().slice(0,10);
  return state.homeCinemaShownDate===today;
}
function markHomeCinemaShown(){
  state.homeCinemaShownDate=new Date().toISOString().slice(0,10);save();
}
function homePosterInner(moment){
  const photo=moment?itemImage(moment):IMAGES.hero;
  ...
  if(playCinema)markHomeCinemaShown();
  const image=`<div class="home-poster-image" style="background-image:linear-gradient(190deg,rgba(5,6,6,.15) 0%,rgba(5,6,6,.35) 55%,rgba(5,6,6,.92) 100%),url('${photo}')"></div>`;
  if(!playCinema){
    // Déjà vue aujourd'hui : on va droit à la révélation, avec une transition courte, jamais le
    // spectacle complet une seconde fois.
    return `${image}<div class="home-poster-content home-poster-instant"><p class="home-poster-kicker">${esc(sequence.reveal)}</p><h1 class="home-poster-title">${esc(title)}</h1><button class="home-poster-cta" onclick="${action}">${esc(cta)}<span>→</span></button></div><div class="home-scroll-hint" aria-hidden="true"><span></span></div>`;
  }
  const beatsHtml=sequence.beats.map((line,index)=>`<p class="home-beat b${index+1}">${esc(line)}</p>`).join('');
  return `${image}<span class="home-signature">${esc(sequence.signature)}</span><div class="home-beats">${beatsHtml}</div><div class="home-poster-content home-poster-reveal"><p class="home-poster-kicker">${esc(sequence.reveal)}</p><h1 class="home-poster-title">${esc(title)}</h1><button class="home-poster-cta" onclick="${action}">${esc(cta)}<span>→</span></button></div><button class="home-skip" onclick="skipHomeCinema()">Passer</button><div class="home-scroll-hint" aria-hidden="true"><span></span></div>`;
}
function skipHomeCinema(){
  document.querySelectorAll('.home-signature,.home-beats').forEach(el=>el.remove());
  document.querySelector('.home-poster-content')?.classList.add('home-poster-instant');
  document.querySelector('.home-skip')?.remove();
}
function homePhrase(){
  ...
  let pool;
  if(/rain|drizzle|thunder/.test(weather))pool=['Il pleut… mais j’ai une idée.','La pluie ne doit pas gâcher la journée.','Un temps parfait pour une autre idée.'];
  else if(hour<11)pool=['Bonjour. Le temps s’ouvre devant vous.','Un matin de plus à ne pas laisser filer.','Vous avez un peu de temps aujourd’hui ?'];
  else if(hour>=19)pool=['La soirée peut encore surprendre.','Il reste de belles heures devant vous.','On change un peu des habitudes ?'];
  else if(/clear|sun/.test(weather))pool=['Il fait beau, profitons-en.','Une belle journée mérite un beau moment.'];
...
async function loadHomePulse(){
  ...
  const emptyPulse=()=>`<div class="pulse-empty"><span>Le champ des possibles reste ouvert</span><strong>Dolcia cherche uniquement des rendez-vous assez fiables.</strong><p>Les deux parcours essentiels restent juste au-dessus : un programme entièrement composé ou des idées filtrées à choisir une par une.</p></div>`;
  try{
...
function renderHomeMajor(moment){const target=document.querySelector('#homePoster');if(!target)return;target.innerHTML=homePosterInner(moment)}
...
function startCompose(){openEclatDialogue(false,'compose')}
...
function setDate(mode){const now=new Date();state.dateMode=mode;let d=new Date(now),end;if(mode==='now'){state.answers.duration='2h';end=new Date(d.getTime()+2*3600000)}if(mode==='tonight'){d.setHours(Math.max(18,now.getHours()),now.getMinutes(),0,0);state.answers.duration='evening';end=new Date(d);end.setHours(23,0,0,0)}if(mode==='tomorrow'){d.setDate(d.getDate()+1);d.setHours(9,0,0,0);state.answers.duration='day';end=new Date(d);end.setHours(18,0,0,0)}if(mode==='weekend'){const add=(6-d.getDay()+7)%7;d.setDate(d.getDate()+add);d.setHours(9,0,0,0);state.answers.duration='stay';end=new Date(d.getFullYear(),d.getMonth(),d.getDate()+1,18,0)}state.dateStart=d;state.dateEnd=end||new Date(d.getTime()+2*3600000);state.month=new Date(d.getFullYear(),d.getMonth(),1);renderDate()}
function setMomentTime(which,value){if(!/^\d{2}:\d{2}$/.test(value))return;const [hours,minutes]=value.split(':').map(Number),target=which==='start'?state.dateStart:state.dateEnd;target.setHours(hours,minutes,0,0);if(state.dateEnd<=state.dateStart){if(which==='start')state.dateEnd=new Date(state.dateStart.getTime()+2*3600000);else return showToast('La fin doit être postérieure au début')}state.dateMode='custom';renderDate()}
...
async function openExplorer(force=false){
  if(!state.momentQualified)return openEclatDialogue(false,'explore');
...
async function compose(explorerOnly=false){
  ...
  try{
    ...
    const officialTouquet=`/api/events?service=touquet&after=${iso(state.dateStart)}&before=${iso(state.dateEnd)}`;
    const ticketmaster=`/api/events?service=ticketmaster&lat=${state.location.lat}&lng=${state.location.lng}&radius=${Math.round(state.radius/1000)}&after=${iso(state.dateStart)}&before=${iso(state.dateEnd)}`;
    const partners=`/api/events?service=partner-events&lat=${state.location.lat}&lng=${state.location.lng}&radius=${Math.round(state.radius/1000)}&after=${iso(state.dateStart)}&before=${iso(state.dateEnd)}`;
    const nationalTourism=`/api/events?service=datatourisme&lat=${state.location.lat}&lng=${state.location.lng}&radius=${Math.round(state.radius/1000)}&after=${iso(state.dateStart)}&before=${iso(state.dateEnd)}`;
    const majorMoments=`/api/events?service=major-events&date=${iso(state.dateStart)}&days=30&lat=${state.location.lat}&lng=${state.location.lng}`;
  ...
  explorerOnly?renderResults():renderSurprise();
}
...
// Classement désir/surprise — ARCHITECTURE.md §5.2-5.4. Seuls les signaux réellement calculables
// avec les données existantes sont implémentés ici ; les autres (type d'expérience jamais proposé
// récemment, différent des habitudes récentes) demandent un historique de propositions pas encore
// suivi — mieux vaut un score honnête sur 3 signaux qu'un score gonflé sur 5 dont 2 seraient devinés.
function desireScore(item){
  let score=0;
  if((item.rating||0)>=4.6&&(item.reviews||0)>=50&&(item.official||item.quality==='verified'||item.ranking?.confidence==='confirmed'))score++; // Sélection Dolcia
  if(item.geoEligibility?.status==='extended')score++; // pépite prouvée
  if(item.official)score++; // source officielle
  if(item.date&&item.timeKnown!==false){const days=(new Date(item.date)-Date.now())/86400000;if(days>=0&&days<=14)score++} // événement limité dans le temps
  return score;
}
function surpriseScore(item){
  if(desireScore(item)<1)return 0; // la surprise amplifie un désir déjà réel, ne le remplace jamais (§5.3)
  let score=0;
  if(!state.experienceMemories[item.id]&&!state.favorites.includes(item.id))score++; // jamais consulté/vécu
  if(item.geoEligibility?.status==='extended'&&Number(item.distance)>5)score++; // hors zone habituelle, preuve de rareté déjà vérifiée
  return score;
}
function scenarioRank(a,b){
  return desireScore(b)-desireScore(a)
    ||surpriseScore(b)-surpriseScore(a)
    ||(a.date&&b.date?new Date(a.date)-new Date(b.date):Number(Boolean(b.date))-Number(Boolean(a.date)))
...
function catalogSelection(){
  ...
  if(filters.budget&&filters.budget!=='all')items=items.filter(item=>item.free||item.freeAccess||(Number.isInteger(item.price)&&item.price<=Number(filters.budget)));
  ...
  if(filters.sort==='recommended')items.sort((a,b)=>scenarioRank(a,b)||(geoOrder[a.geoEligibility?.status]??2)-(geoOrder[b.geoEligibility?.status]??2)||(momentOrder[momentCompatibility(a)]??9)-(momentOrder[momentCompatibility(b)]??9)||(filters.preferred.length?Number(filters.preferred.includes(catalogFamily(b)))-Number(filters.preferred.includes(catalogFamily(a))):0)||(a.geoEligibility?.travel_minutes??999)-(b.geoEligibility?.travel_minutes??999)||(b.score||0)-(a.score||0));
...
function zeroResultRecovery({sourceDown=false,embedded=false}={}){
  ...
  const couple=state.answers.who==='couple',smallBudget=couple&&(state.answers.budget==='free'||(state.answers.budget==='custom'&&Number(state.budgetPlan.amount)<=60));
  const freeLocal=[...state.allItems].filter(item=>(item.free||item.price===0||item.freeAccess)&&geoVisible(item)&&recommendationEligibleNow(item)).sort((a,b)=>(a.distance??999)-(b.distance??999)||(b.score||0)-(a.score||0)).slice(0,3);
  const title=sourceDown?'Les sources répondent mal. Votre moment, lui, peut commencer.':smallBudget?'Votre budget n’est pas le problème. On peut vivre un beau moment à deux.':freeMoment&&socialLaugh?'À 0 €, entre amis, D a déjà de quoi vous faire rire.':'Aucune correspondance exacte. Jamais aucune solution.';
  const intro=sourceDown?'Je peux relancer les sources sans perdre vos choix, ou lancer immédiatement une animation gratuite avec D.':smallBudget?'Je garde vos 60 € maximum. Voici d’abord les accès libres réellement documentés, puis une expérience complice gratuite guidée par D.':'Choisissez vous-même le seul critère à assouplir. Dolcia ne change jamais votre budget ni votre zone sans votre accord.';
  return `<section class="zero-cascade ${embedded?'embedded':''}">
    ...
    <div class="zero-cascade-options">
      ${freeLocal.length?`<button class="recovery-option recommended" onclick="showFreeLocalIdeas()"><small>01 · Gratuit ou accès libre · tout près</small><strong>Voir ${freeLocal.length} idée${freeLocal.length>1?'s':''} sans dépasser le budget</strong><span>${freeLocal.map(item=>esc(item.name)).join(' · ')}</span></button>`:''}
      ...
      <button class="recovery-option animate ${(freeMoment&&socialLaugh)||smallBudget?'recommended':''}" onclick="${smallBudget?"openDolciaAnimate('animate');setTimeout(()=>previewDolciaAnimate('rdv_complice'),80)":"openDolciaAnimate()"}"><small>${sourceDown?(next?'04':'03'):(next?'03':'02')} · Gratuit · sans trajet imposé</small><strong>${smallBudget?'Le rendez-vous complice avec D':'D anime votre moment'}</strong><span>${smallBudget?'35 minutes à deux, guidées avec empathie par D : souvenirs, jeu complice, compliments et prochaine envie commune.':socialLaugh?'Défis, jeux connus et fous rires guidés entre amis au Touquet — D accueille, rythme, adapte et conclut.':'D prend en charge l’animation complète, de l’accueil à la conclusion, au rythme de votre groupe.'}</span></button>
    </div>
...
function showFreeLocalIdeas(){
  resetMomentCatalogFilters();
  state.catalogFilters.lenses=['free'];
  state.catalogFilters.sort='distance';
  renderResults();
  showToast('Dolcia affiche les idées gratuites ou en accès libre, sans modifier votre budget.');
}
...
function clarifyExplorerUi(){
  const copy=document.querySelector('.explore-signature-copy');
  if(copy)copy.innerHTML=`<span class="kicker">Vos idées adaptées · ${esc(state.location.name)}</span><h1>Choisissez ce qui<br><em>vous fait vraiment envie.</em></h1><p>Cette sélection tient déjà compte de votre date, du groupe, de la durée, de l’envie et du budget. Vous gardez la main et ajoutez uniquement ce qui vous plaît.</p>`;
  const composeButton=document.querySelector('.signature-compose');
  if(composeButton)composeButton.innerHTML=`<span>Passer au programme complet</span><strong>${composeCta()}</strong><b>→</b>`;
  const adjust=document.querySelector('.signature-adjust');
  if(adjust){adjust.textContent='Modifier mon contexte';adjust.setAttribute('onclick',"openEclatDialogue(false,'explore')")}
}
...
function adaptiveLensDefinitions(){return[
  ...
  {id:'pepite',label:'Pépite locale',test:item=>item.geoEligibility?.status==='extended'},
  {id:'sheltered',label:'À l’abri de la pluie',test:item=>!['outside','water'].includes(item.category)},
  ...
  {id:'rare',label:'Rarement vu ailleurs',test:item=>(item.reviews||0)>0&&(item.reviews||0)<=15&&(item.rating||0)>=4.3},
...
// Groupement par moment de la journée — répond directement au besoin de construire une journée
// (activité du matin, déjeuner à proximité, activité de l'après-midi, dîner le soir) sans se
// perdre dans une liste plate. Les restaurants ont leur propre section : rien dans les données ne
// permet de savoir si un restaurant donné est plutôt un lieu de midi ou de soir, donc Dolcia ne le
// devine jamais — la personne choisit elle-même selon l'heure qui l'intéresse.
function renderCatalogByDaypart(items){
  const sections=[
    {label:'Le matin', categories:['outside','active','culture']},
    {label:'Pour manger — midi ou soir, à vous de choisir', categories:['food']},
    {label:"L'après-midi", categories:['outside','active','culture','slow']},
    {label:'Le soir', categories:['night']}
  ];
  const used=new Set();
  const blocks=sections.map(section=>{
    const sectionItems=items.filter(item=>section.categories.includes(item.category)&&!used.has(item.id));
    sectionItems.forEach(item=>used.add(item.id));
    if(!sectionItems.length)return'';
    return `<div class="daypart-section"><h3>${esc(section.label)}<small>${sectionItems.length}</small></h3><div class="catalog-list">${sectionItems.map((item,index)=>experience(item,index,section.label)).join('')}</div></div>`;
  }).filter(Boolean).join('');
  const remaining=items.filter(item=>!used.has(item.id));
  const remainingBlock=remaining.length?`<div class="daypart-section"><h3>Autres idées<small>${remaining.length}</small></h3><div class="catalog-list">${remaining.map((item,index)=>experience(item,index,'')).join('')}</div></div>`:'';
  return blocks+remainingBlock;
}
...
function clarifyProgramUi(){
  ...
  document.querySelector('.essential-nav .signature')?.classList.add('active');
  document.querySelectorAll('.surprise-controls button').forEach(button=>{
    const label=plainText(button.textContent||'');
    if(label.includes('adopter ce programme'))button.textContent='Ajouter tout à mon agenda';
    else if(label.includes('tout recomposer'))button.textContent='Régénérer tout le programme';
    else if(label.includes('explorer toutes les idées'))button.textContent='Choisir activité par activité';
  });
  document.querySelectorAll('.program-slot button').forEach(button=>{
    const label=plainText(button.textContent||'');
    if(label==='remplacer')button.textContent='Changer cette activité';
    else if(label==='affiner')button.textContent='Préciser cette étape';
  });
}
function renderSurprise(){
  state.view='surprise';
  const criteria=criteriaLabels();
  setTimeout(clarifyProgramUi,0);
...
function astronomyProgramPanel(){if(!state.majorChoice)return'';const moment=state.majorMoments.find(item=>item.id===state.majorChoice);if(moment?.kind!=='astronomy')return'';return`<aside class="program-broadcast astronomy-sites"><span>Où observer ce phénomène ?</span><strong>Des sites réels proches, jamais une visibilité garantie</strong><p>Dolcia sépare l’existence du lieu de la qualité d’observation : météo, horizon, accès et affluence restent à confirmer le jour même.</p>${observationVenueList()}</aside>`}
...
function criteriaLabels(){const vibeStep=FLOW.find(step=>step.key==='vibes');const vibes=(state.answers.vibes||[]).map(value=>vibeStep.options.find(option=>option[0]===value)?.[1]).filter(Boolean),budget=state.answers.budget==='custom'&&state.budgetPlan.amount!=null?`${state.budgetPlan.amount.toLocaleString('fr-FR')} € pour le groupe`:label('budget'),start=state.dateStart.toLocaleString('fr-FR',{day:'numeric',month:'long',hour:'2-digit',minute:'2-digit'}),end=state.dateEnd.toLocaleString('fr-FR',{day:tripDays()>1?'numeric':undefined,month:tripDays()>1?'long':undefined,hour:'2-digit',minute:'2-digit'}),people=currentGroupProfiles().map(person=>person.name).join(', ');return [`${start} → ${end}`,people||label('who'),budget,...vibes].filter(Boolean)}
function activityDurationPresentation(item){
  if(item.autonomousProgram&&Number(item.duration||DOLCIA_ANIMATE_PROGRAMS[item.autonomousProgram]?.duration))return{minutes:Number(item.duration||DOLCIA_ANIMATE_PROGRAMS[item.autonomousProgram].duration),status:'confirmed',label:`${Number(item.duration||DOLCIA_ANIMATE_PROGRAMS[item.autonomousProgram].duration)} min · durée du programme`};
  if(item.date&&item.endDate){const minutes=Math.round((new Date(item.endDate)-new Date(item.date))/60000);if(minutes>0&&minutes<1440)return{minutes,status:'confirmed',label:`${minutes>=60?`${Math.floor(minutes/60)} h${minutes%60?` ${minutes%60}`:''}`:`${minutes} min`} · confirmé par la source`}}
  if(Number(item.durationMinutes)>0)return{minutes:Number(item.durationMinutes),status:item.durationKnown===false?'estimated':'confirmed',label:`${Number(item.durationMinutes)} min · ${item.durationKnown===false?'estimation Dolcia':'confirmé'}`};
  const estimates={food:90,culture:90,outside:120,active:75,fun:75,slow:90,wellness:90,night:120,hotel:0},minutes=estimates[item.category]||90;
  return minutes?{minutes,status:'estimated',label:`Environ ${minutes>=60?`${Math.floor(minutes/60)} h${minutes%60?` ${minutes%60}`:''}`:`${minutes} min`} · estimation Dolcia`}:{minutes:0,status:'unknown',label:'Durée à confirmer'}
}
...
function experience(i,index,slotLabel=''){const added=state.agenda.some(item=>item.id===i.id),reserved=state.reservations.some(r=>r.itemId===i.id&&r.status!=='cancelled'),fallback=IMAGES[i.category]||IMAGES.fallback,label=i.partner?'Partenaire Dolcia · réservation intégrée':i.sponsored?'Suggestion partenaire · pertinence vérifiée':i.quality==='verified'?'Informations vérifiées':i.quality==='official-partial'?'Source officielle · détails à confirmer':esc(i.source),trust=confidencePresentation(i),price=i.free?'<span>Gratuit confirmé</span>':i.freeAccess?'<span>Accès libre · options éventuelles payantes</span>':i.price!=null?`<span>${i.price===0?'Gratuit confirmé':'€'.repeat(Math.min(i.price,4))}</span>`:'<span>Tarif non communiqué</span>',dateMeta=i.date?`<span>${i.timeKnown===false?new Date(i.date).toLocaleDateString('fr-FR',{day:'numeric',month:'long'})+' · horaire à confirmer':new Date(i.date).toLocaleString('fr-FR',{day:'numeric',month:'long',hour:'2-digit',minute:'2-digit'})}</span>`:'',durationMeta=`<span class="duration-${activityDurationPresentation(i).status}">${esc(activityDurationPresentation(i).label)}</span>`,travel=i.geoEligibility?.travel_minutes,travelMeta=travel!=null?`<span>${travel} min de trajet</span>`:i.distance!=null?`<span>${i.distance.toFixed(1)} km à vol d’oiseau</span>`:'';const hearts=scenarioHearts(i),reasons=whyTodayReasons(i);
  return `<article class="experience ${i.partner?'partner-ready':''} ${i.geoEligibility?.status==='outside'?'outside-scope':''}"><span class="scenario-hearts" aria-label="${hearts} sur 5">${'❤'.repeat(hearts)}${'♡'.repeat(5-hearts)}</span><button class="quick-heart ${state.favorites.includes(i.id)?'selected':''}" onclick="openReactionMenu('${i.id}')" aria-label="Dire à Dolcia si cette activité vous plaît">${state.favorites.includes(i.id)?'♥':'♡'}</button><img class="exp-image fade-img" src="${itemImage(i)}" alt="" loading="lazy" onload="this.classList.add('img-loaded')" onerror="this.onerror=null;this.src='${fallback}'"><div class="exp-copy"><span class="exp-label">${label}<i class="trust-badge ${trust[0]}">${trust[1]}</i></span><h3>${esc(i.name)}</h3><p>${esc(i.address||'Lieu exact à confirmer auprès de la source')}</p><div class="exp-meta">${travelMeta}${durationMeta}${i.rating?`<span>Google ${i.rating}/5${i.reviews?` · ${i.reviews} avis`:''}</span>`:''}${price}${i.isOpen===true?'<span>Ouvert actuellement</span>':''}${dateMeta}</div><div class="why-today"><b>Pourquoi aujourd’hui ?</b><ul>${reasons.map(reason=>`<li>${esc(reason)}</li>`).join('')}</ul></div></div><div class="exp-actions clear-actions"><button class="detail-button" onclick="openDetail('${i.id}')">Voir la fiche</button>${i.partner?`<button class="agenda-button pass-action ${reserved?'added':''}" onclick="reserveWithDolcia('${i.id}','${esc(slotLabel)}')">${reserved?'Réservation confirmée':'Réserver avec Dolcia'}</button>`:`<button class="agenda-button ${added?'added':''}" onclick="addAgenda('${i.id}','${esc(slotLabel)}')">${added?'Ajouté à mon agenda':'Ajouter à mon agenda'}</button>`}</div></article>`}
...
function scenarioHearts(item){return Math.max(1,Math.min(5,desireScore(item)+surpriseScore(item)))}
function whyTodayReasons(item){
  const reasons=[];
  if((item.rating||0)>=4.6&&(item.reviews||0)>=50&&(item.official||item.quality==='verified'||item.ranking?.confidence==='confirmed'))reasons.push('Sélection Dolcia');
  if(item.geoEligibility?.status==='extended')reasons.push('Pépite vérifiée');
  if(item.date&&item.timeKnown!==false){const days=(new Date(item.date)-Date.now())/86400000;if(days>=0&&days<1)reasons.push('Seulement aujourd’hui');else if(days>=0&&days<=14)reasons.push('Le moment de ne pas rater');}
  if(!state.experienceMemories[item.id]&&!state.favorites.includes(item.id)&&desireScore(item)>=1)reasons.push('Jamais vécu par vous');
  if(item.geoEligibility?.status==='extended'&&Number(item.distance)>5)reasons.push('Ça vaut le déplacement');
  if(!reasons.length)reasons.push(why(item));
  return reasons;
}
function requiresTideAwareness(item){
  // Vérifié : aucune donnée de marée réelle n'est branchée dans ce projet. Plutôt que de deviner
  // un horaire ou de rester silencieux, cette fonction signale honnêtement que l'activité dépend
  // des marées — sans jamais prétendre savoir à quelle heure la marée sera favorable. Le paddle et
  // le kayak ont besoin de suffisamment d'eau ; le char à voile a besoin d'une plage découverte :
  // deux besoins opposés, raison de plus pour ne jamais affirmer un horaire sans vérification
  // réelle auprès d'un vrai club ou d'une vraie table de marées.
  return /paddle|kayak|catamaran|\bvoile\b|kite.?surf|wing.?foil|e.?foil|char[- ]?à[- ]?voile|char[- ]?a[- ]?voile/i.test(`${item.name||''} ${(item.types||[]).join(' ')}`)&&!/piscine|bassin|lac\b/i.test(`${item.name||''}`);
}
// Lien réel et vérifié vers les horaires de marée du Touquet (marée.info, port n°8, confirmé par
// recherche directe — pas une supposition, pas un identifiant deviné). Le Touquet reste le seul
// territoire pilote de ce projet : ce lien n'est volontairement pas généralisé à d'autres villes
// tant que leur identifiant réel n'a pas été vérifié de la même façon — un lien vers une recherche
// générique reste le repli honnête plutôt qu'un identifiant de port inventé.
function tideScheduleLink(){
  if(/touquet/i.test(state.location?.name||''))return'https://maree.info/8';
  // Repli honnête : je n'ai vérifié aucun paramètre de recherche pour d'autres villes sur ce
  // site. Plutôt que de deviner une URL qui pourrait ne pas fonctionner, ce lien mène à la page
  // d'accueil réelle, vérifiée, où la personne peut chercher son port elle-même.
  return'https://maree.info/';
}
function why(i){const temp=state.weather?.main?.temp??18,vibes=state.answers.vibes||[],compatibility=momentCompatibility(i),group=i.ranking?.groupFit;if(geoWhy(i))return geoWhy(i);if(compatibility==='incompatible')return'Incompatible avec le créneau choisi';if(requiresTideAwareness(i))return'Dépend des marées · vérifiez l’horaire réel auprès du club avant de vous engager';if(compatibility==='unknown'||compatibility==='open-not-session')return requiresPublishedSession(i)?'Séance ou réservation à confirmer':'Horaire à vérifier';if(group?.vetoes)return'Une réserve existe dans votre groupe · visible dans Explorer, exclue du programme composé';if(group?.ratio>=.75)return`Point d’accord fort · ${group.matched} affinités reconnues dans votre groupe`;if(compatibility==='autonomous')return'Accès libre possible · conditions à vérifier';if(i.retrievalScope==='signature')return [`Expérience régionale repérée pour votre journée`,i.geoEligibility?.travel_minutes!=null?`${i.geoEligibility.travel_minutes} min de trajet`:null,`le déplacement reste indiqué clairement`].filter(Boolean).join(' · ');if(compatibility==='confirmed'&&i.source==='Google Places')return [`Ouvert pour votre créneau`,i.geoEligibility?.travel_minutes!=null?`${i.geoEligibility.travel_minutes} min de trajet`:null,i.rating?`${i.rating}/5 · ${i.reviews||0} avis`:null].filter(Boolean).join(' · ');if(i.ranking?.reasons?.length)return i.ranking.reasons.map(reason=>reason.charAt(0).toUpperCase()+reason.slice(1)).join(' · ');if(i.discoveryPick)return'La pépite que vous n’auriez peut-être pas cherchée';if(i.official&&i.date)return'Événement officiel à votre date';if(temp>=25&&/plage|nautique|voile|surf|paddle|kayak|piscine/.test(plainText(i.name||'')))return'Idéal avec la météo du moment';if((state.tasteProfile[i.category]||0)>1)return'Inspiré de ce que vous aimez déjà';if(matchesIntentions(i,vibes))return'Correspond à votre envie maintenant';if(i.rating>=4.6&&i.reviews>=100)return'Valeur sûre très appréciée';if(i.distance!=null&&i.distance<2)return'Belle idée tout près de vous';return'Sélection Dolcia pour élargir vos envies'}
...
function emptyState(){return `<div class="empty-state"><span class="kicker">Toujours une alternative</span><h3>Le catalogue n’a pas encore livré la bonne idée.</h3><p>Dolcia peut relancer les sources, affiner votre envie ou créer une expérience autonome sans inventer un événement.</p><div class="empty-actions"><button class="primary" onclick="surprise()">${composeCta()}</button><button class="secondary" onclick="openDolciaAnimate()">Dolcia anime ce moment</button></div></div>`}

const DOLCIA_ANIMATE_PROGRAMS={
  sport:{title:'Cardio Club avec D',tone:'Une vraie séance cardio, fun et fédératrice',duration:35,place:'Espace dégagé, plage, parc ou pelouse',phased:true,coach:true,steps:[
    ['00:00','Bienvenue au Club Dolcia ! Marche active, épaules relâchées, puis talons-fesses légers. D lance les prénoms : chacun entraîne le groupe pendant vingt secondes.',{phase:'echauffement'}],
    ['00:06','Premier jeu : feu vert, feu orange ! Trente secondes de pas rapides, quinze secondes de récupération. D change le signal et surprend le groupe sur quatre manches.',{phase:'echauffement'}],
    ['00:12','Circuit Club, manche 1 : huit squats confortables, huit montées de genoux par côté, puis déplacement latéral jusqu’au repère. Deux tours, avec option sans saut.',{phase:'coeur'}],
    ['00:20','Défi duo : pendant que l’un fait des pas rapides, l’autre compte et encourage ; on échange après trente secondes. Trois manches, objectif collectif annoncé par D.',{phase:'coeur'}],
    ['00:27','Finale sourire : vingt secondes de jumping jacks ou pas chassés sans saut, dix secondes pour souffler, quatre fois. D fait le compte à rebours et célèbre chaque manche.',{phase:'coeur'}],
    ['00:32','Retour au calme : marche lente, respiration, mollets et cuisses relâchés. Chacun partage son moment préféré ; D conclut avec le prénom de tous.',{phase:'retour'}]
  ]},
  ...
  renforcement:{title:'Le Challenge Tonique de D',tone:'Renforcement ludique sans matériel',duration:30,place:'Un tapis, une pelouse ou un salon dégagé',phased:true,coach:true,steps:[
    ['00:00','D accueille l’équipe : marche dynamique, cercles d’épaules et huit assis-debout contrôlés. Chacun choisit son option, jamais de douleur ni de posture forcée.',{phase:'echauffement'}],
    ['00:05','Manche des prénoms : six squats confortables, puis on passe symboliquement le relais à la personne suivante. Deux tours et beaucoup d’encouragements.',{phase:'echauffement'}],
    ['00:10','Circuit 40/20 : fentes arrière courtes ou pas arrière, pompes au mur ou sur appui, puis gainage debout mains contre mains. D chronomètre trois tours.',{phase:'coeur'}],
    ['00:19','Défi miroir en duo : l’un invente trois mouvements simples, l’autre reproduit. Trente secondes chacun, puis revanche avec davantage d’énergie.',{phase:'coeur'}],
    ['00:25','Finale collective : dix squats ou assis-debout au total par personne. D compte, relance et transforme chaque répétition en point d’Élan du groupe.',{phase:'coeur'}],
    ['00:28','On relâche les jambes et les épaules, respiration lente. D demande comment chacun se sent et retient la variante préférée pour la prochaine séance.',{phase:'retour'}]
  ]},
...
function refineAnimateChoice(intent){
  const map={move:'sport',calm:'calm',laugh:'party',together:'social',family:'family',water:'water'};
  const id=map[intent]||suggestedAnimateProgram();
  previewDolciaAnimate(id);
  document.querySelectorAll('.animate-quick-questions button').forEach(button=>button.classList.toggle('selected',button.getAttribute('onclick')?.includes(`'${intent}'`)));
}
...
function upgradeAnimateLauncher(){
  ...
  if(!article)return;
  article.classList.add('animate-premium-launcher');
  if(!article.querySelector('.animate-cinematic-backdrop'))article.insertAdjacentHTML('afterbegin','<div class="animate-cinematic-backdrop" aria-hidden="true"></div>');
  const mark=article.querySelector('header > span');
  if(mark)mark.outerHTML=dMascotMark('large');
  const title=article.querySelector('header h2');
  const copy=article.querySelector('header p');
  if(title)title.textContent='D vous parle avant de vous faire bouger';
  if(copy)copy.textContent='D accueille le groupe, explique les règles, forme les équipes, rythme la partie, tient les scores, relance l’énergie et conclut le moment avec vous. En mode Coach, elle lance aussi les exercices, les variantes et les récupérations.';
}
function previewDolciaAnimate(id){
  ...
  target.innerHTML=`<section><div><span class="kicker">${program.coach?'Séance Coach conduite par D':esc(program.tone)}</span><h3>${esc(program.title)}</h3><p>${esc(program.place)} · ${program.duration} minutes · D parle et s’adapte en direct</p></div><div class="animate-start-now"><label><input type="checkbox" id="animateSafety"> On a choisi un endroit sûr, tout le monde est prêt. Chacun choisira sa variante sans douleur.</label><div><button class="secondary" onclick="addDolciaAnimate('${id}')">Garder pour plus tard</button>${program.coach?`<button class="ghost" onclick="startDolciaAnimate('${id}')">Commencer tout de suite, sans les questions</button>`:''}<button class="primary" onclick="${program.coach?`openCoachBrief('${id}')`:`startDolciaAnimate('${id}')`}">${program.coach?'▶ 3 questions rapides, puis on commence':'▶ Commencer avec D'}</button></div></div><ol>${program.steps.map(step=>`<li><time>${step[0]}</time><span>${esc(step[1])}</span></li>`).join('')}</ol></section>`;
...
function openCoachBrief(id){
  if(!ensureAnimateSafety())return;const program=DOLCIA_ANIMATE_PROGRAMS[id];if(!program)return;
  state.coachBrief={id,step:0,answers:{}};renderCoachBrief()
}
function coachBriefQuestion(){return[
  {key:'feeling',line:'Avant de lancer la musique, je veux savoir comment vous arrivez.',title:'Votre énergie, là, maintenant ?',choices:[['boost','On veut se défouler'],['steady','On veut bouger sans se tuer'],['restart','On reprend doucement'],['mixed','On n’a pas tous la même énergie']]},
  {key:'spirit',line:'Parfait. Je vais garder une vraie structure sportive, mais je peux changer complètement l’ambiance.',title:'Qu’est-ce qui vous fera tenir avec le sourire ?',choices:[['game','Des jeux et des surprises'],['team','Un défi d’équipe'],['music','Du rythme et de la musique'],['progress','Sentir qu’on progresse']]},
  {key:'voice',line:'Dernière chose : je reste avec vous pendant toute la séance.',title:'Comment voulez-vous que je vous anime ?',choices:[['club','Comme au Club Med : solaire et très présente'],['challenge','Énergique, avec des comptes à rebours'],['kind','Encourageante et rassurante'],['family','Simple et drôle pour toute la famille']]}
][state.coachBrief?.step||0]}
function renderCoachBrief(){
  document.querySelector('#coachBriefModal')?.remove();const brief=state.coachBrief,q=coachBriefQuestion(),people=currentGroupProfiles().map(person=>person.name).filter(Boolean).join(', ');
  document.body.insertAdjacentHTML('beforeend',`<div class="modal coach-brief" id="coachBriefModal"><article><button class="close" onclick="document.querySelector('#coachBriefModal')?.remove()">×</button><div class="coach-d-stage">${dMascotMark('coach-hero')}<span class="d-live-badge"><i></i>D est avec vous</span></div><div class="coach-dialogue"><small>D · votre coach-animateur${people?' · '+esc(people):''}</small><p>« ${esc(q.line)} »</p><h2>${esc(q.title)}</h2><div>${q.choices.map(([value,label])=>`<button onclick="answerCoachBrief('${q.key}','${value}')">${label}<b>→</b></button>`).join('')}</div><label>Ou répondez à D avec vos mots<input id="coachBriefFree" placeholder="Ex. On est fatigués mais on veut rire…"><button onclick="answerCoachBriefFree('${q.key}')">Dire à D</button></label></div></article></div>`);
  setDVisualState('listening');playPremiumVoice(`${q.line} ${q.title}`,()=>setDVisualState('listening'))
}
function answerCoachBrief(key,value){const brief=state.coachBrief;if(!brief)return;brief.answers[key]=value;if(brief.step<2){brief.step++;return renderCoachBrief()}launchCoachFromBrief()}
function answerCoachBriefFree(key){const value=document.querySelector('#coachBriefFree')?.value.trim();if(!value)return showToast('D vous écoute : dites-lui simplement comment vous arrivez');answerCoachBrief(key,value)}
function launchCoachFromBrief(){const brief=state.coachBrief;if(!brief)return;document.querySelector('#coachBriefModal')?.remove();startDolciaAnimate(brief.id,brief.answers)}
function dolciaAnimateItem(id){
  const program=DOLCIA_ANIMATE_PROGRAMS[id];return{id:`dolcia-animate-${id}-${Date.now()}`,name:program.title,tone:program.tone,category:id==='calm'?'slow':id==='water'?'outside':'active',source:'Programme Dolcia',address:program.place,summary:`Expérience autonome guidée par Dolcia · ${program.duration} minutes`,duration:program.duration,durationMinutes:program.duration,durationKnown:true,free:true,price:0,detailsKnown:true,autonomousProgram:id,steps:program.steps,quality:'verified',official:false,photo:IMAGES[id==='family'?'family':id==='calm'?'slow':id==='water'?'outside':'friends']}
}
...
// conformément à la règle déjà posée : l'Élan ne classe jamais les personnes.
const TEAM_NAMES = [['Les Soleils','Les Embruns'],['Les Corsaires','Les Flibustiers'],['Team Vague','Team Dune'],['Les Complices','Les Alliés']];
function buildTeams(roster){
  if(roster.length<3||roster[0]==='Le groupe')return null;
  const shuffled=[...roster].sort(()=>Math.random()-.5);
  const mid=Math.ceil(shuffled.length/2);
  const [nameA,nameB]=TEAM_NAMES[Math.floor(Math.random()*TEAM_NAMES.length)];
  return{a:{name:nameA,members:shuffled.slice(0,mid),score:0},b:{name:nameB,members:shuffled.slice(mid),score:0},turn:'a'};
}
function animateTeamsBar(session){
  if(!session.teams)return'';
  const{a,b,turn}=session.teams;
  return`<div class="animate-teams"><div class="${turn==='a'?'active':''}"><b>${a.score}</b><span>${esc(a.name)}</span></div><div class="animate-teams-vs">vs</div><div class="${turn==='b'?'active':''}"><b>${b.score}</b><span>${esc(b.name)}</span></div></div>`;
}
// Partage de session : une personne "prend la main" (avance les étapes), les autres suivent en
// direct sur leur propre téléphone via un code court ou un QR — comme un vrai animateur de club où
// personne n'a besoin de se serrer autour d'un seul écran. La famille déjà dans le Cercle n'a jamais
// besoin de ce code. Il sert uniquement aux amis externes, pour une durée choisie et limitée.
const SHARE_DURATIONS=[{id:'24h',label:'24 heures'},{id:'2j',label:'2 jours'},{id:'3j',label:'3 jours'},{id:'1semaine',label:'1 semaine'}];
async function shareAnimateSession(duration){
  const session=state.activeAnimate;if(!session)return;
  try{
    const response=await fetch('/api/events?service=animate-session',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'create',duration,state:animateSessionSnapshot(session)})});
    const data=await response.json().catch(()=>({}));
    if(!response.ok||!data.code)return showToast('Impossible de créer le code de partage pour le moment.');
    session.shareCode=data.code;session.shareExpiresAt=data.expiresAt;
    save();renderDolciaAnimateLive();
    showToast(`Code créé : ${data.code} — valable ${SHARE_DURATIONS.find(d=>d.id===duration)?.label||'24 heures'}`);
  }catch{showToast('Impossible de créer le code de partage pour le moment.')}
}
function animateSessionSnapshot(session){
  return{name:session.item.name,tone:session.item.tone,index:session.index,steps:session.item.steps,phased:Boolean(session.item.phased),elan:session.elan,teams:session.teams,captain:animateCaptain(session),energyTheme:animateEnergyTheme(session).id};
}
function pushAnimateSessionState(){
  const session=state.activeAnimate;if(!session?.shareCode)return;
  fetch('/api/events?service=animate-session',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({action:'update',code:session.shareCode,state:animateSessionSnapshot(session)})}).catch(()=>{});
}
function stopShareAnimateSession(){const session=state.activeAnimate;if(session){session.shareCode=null;session.shareExpiresAt=null;save();renderDolciaAnimateLive()}}
function animateSharePanel(session){
  if(!session.shareCode)return'';
  const joinUrl=`${window.location.origin}/?join=${session.shareCode}`;
  const qrSrc=`https://api.qrserver.com/v1/create-qr-code/?size=120x120&margin=8&data=${encodeURIComponent(joinUrl)}`;
  const invited=(session.invitedContacts||[]).length;
  return`<div class="animate-share-panel"><div><span class="kicker">Suivez sur votre téléphone</span><strong>${session.shareCode}</strong><p>Envoyez le lien par SMS ou email — un clic pour votre ami, il reste dans ses messages, pas besoin de sortir l'appareil photo.</p>
    <div class="share-contact-row"><input id="animateShareContact" placeholder="Numéro ou email de votre ami" inputmode="email"><button class="share-link-button" onclick="sendAnimateInvite('${joinUrl}','${session.shareCode}')">Envoyer</button></div>
    ${invited?`<small class="share-invited-count">${invited} invitation${invited>1?'s':''} envoyée${invited>1?'s':''}</small>`:''}
    <div class="share-fallback-row"><button onclick="shareAnimateLink('${joinUrl}','${session.shareCode}')">Ou partager le lien autrement</button><small class="share-qr-hint">Vous êtes ensemble ? Scannez :</small></div></div>
    <img src="${qrSrc}" alt="QR code pour rejoindre la session" width="64" height="64"></div>`;
}
// Méthode principale, façon Spotify Jam (SMS/email direct, un clic, reste dans les messages du
// destinataire) — plus fluide qu'un QR quand on est déjà en interaction avec la personne, et ça
// capture au passage ses coordonnées pour le Cercle. Le lien natif et le QR restent en repli :
// le QR surtout utile en solution de secours (affiche, chevalet), pas comme méthode principale.
function sendAnimateInvite(joinUrl,code){
  const input=document.querySelector('#animateShareContact'),contact=(input?.value||'').trim();
  if(!contact)return showToast('Entrez un numéro ou un email d\u2019abord.');
  const session=state.activeAnimate;
  const message=`Suivez notre moment Dolcia en direct sur votre téléphone (code ${code}) : ${joinUrl}`;
  const isEmail=contact.includes('@');
  if(isEmail){
    window.location.href=`mailto:${encodeURIComponent(contact)}?subject=${encodeURIComponent('Rejoignez notre session Dolcia')}&body=${encodeURIComponent(message)}`;
  }else{
    window.location.href=`sms:${encodeURIComponent(contact)}?body=${encodeURIComponent(message)}`;
  }
  if(session){session.invitedContacts=session.invitedContacts||[];session.invitedContacts.push({contact,type:isEmail?'email':'sms',sentAt:new Date().toISOString()});save()}
  if(input)input.value='';
  renderDolciaAnimateLive();
}
// Repli, pas la méthode principale : partage natif générique (WhatsApp, tout autre canal), utile
// si la personne préfère un autre outil que SMS/email.
async function shareAnimateLink(joinUrl,code){
  const shareData={title:'Rejoignez ma session Dolcia',text:`Suivez notre moment Dolcia en direct sur votre téléphone — code ${code}`,url:joinUrl};
  try{
    if(navigator.share){await navigator.share(shareData);return}
  }catch{return}
  try{await navigator.clipboard.writeText(joinUrl);showToast('Lien copié — collez-le où vous voulez')}
  catch{showToast(`Lien : ${joinUrl}`)}
}
function openAnimateSharePicker(){
  const session=state.activeAnimate;if(!session)return;
  document.querySelector('#animateSharePicker')?.remove();
  document.body.insertAdjacentHTML('beforeend',`<div class="modal" id="animateSharePicker"><article class="animate-share-picker"><button class="close" onclick="document.querySelector('#animateSharePicker')?.remove()">×</button><h3>Partager avec des amis</h3><p>Votre famille déjà dans votre Cercle n'a besoin d'aucun code. Ce code sert uniquement à des amis externes, pour une durée limitée.</p><div class="share-duration-choices">${SHARE_DURATIONS.map(d=>`<button onclick="document.querySelector('#animateSharePicker')?.remove();shareAnimateSession('${d.id}')">${d.label}</button>`).join('')}</div></article></div>`);
}
function startDolciaAnimate(id,coachProfile=null){
  if(!coachProfile&&!ensureAnimateSafety())return;
  const item=dolciaAnimateItem(id);
  const roster=currentGroupProfiles().map(person=>person.name).filter(Boolean);
  state.activeAnimate={item,index:0,startedAt:new Date().toISOString(),lastInteractionAt:Date.now(),elan:0,completed:0,adjustments:[],reactions:[],status:'active',roster:roster.length?roster:['Le groupe'],captainIndex:0,streak:0,laughs:0,messages:coachProfile?[{role:'assistant',content:'J’ai compris votre énergie et l’ambiance que vous voulez. Je reste avec vous pendant toute la séance.'}]:[],insideJokes:[],nudgeCount:0,teams:buildTeams(roster.length?roster:['Le groupe']),continuity:animateContinuityGreeting(),coachProfile};
  document.querySelector('#dolciaAnimate')?.remove();
...
function animatePersonalTouch(session){
  ...
  if(joke&&session.index>1)return`${captain}, garde « ${joke} » sous le coude : je sens que ça peut revenir au meilleur moment.`;
  return captain==='Le groupe'?'Je reste avec toi et je règle le rythme en direct.':`${captain}, tu es capitaine de cette mission — juste pour lancer le mouvement, pas pour commander la troupe.`;
}
// L'historique des sessions (state.animateHistory) était déjà sauvegardé à chaque fin de moment,
// mais jamais relu — Dolcia Anime "oubliait" tout d'une fois à l'autre malgré la donnée déjà là.
// Accompagner dans la durée, pas seulement pendant un moment : célébrer les jalons, reconnaître
// qu'on se retrouve, jamais inventer un chiffre qui ne vient pas de l'historique réel.
function animateContinuityGreeting(){
  const history=state.animateHistory||[];
  if(!history.length)return null;
  const count=history.length+1;
  const daysSinceLast=Math.floor((Date.now()-new Date(history[0].completedAt).getTime())/86400000);
  if([3,5,10,20,30].includes(count))return`C'est notre ${count}e moment ensemble — sérieusement, bravo pour cette régularité !`;
  if(daysSinceLast>=14)return`Ça faisait un moment ! Contente de te retrouver pour animer ça.`;
  if(daysSinceLast===0&&history.length>0)return`Encore une session aujourd'hui, j'adore cette énergie !`;
  return null;
}
function animateCoachLine(session){
  if(!session.index){
    const continuity=session.continuity;
    const style=session.coachProfile?.voice,feeling=session.coachProfile?.feeling;
    const personal=style==='club'?'Aujourd’hui je prends le micro comme au club : sourire, énergie et personne laissé derrière !':style==='challenge'?'Je garde le chrono et les comptes à rebours, tu gardes le contrôle de l’intensité.':style==='kind'?'Je t’encourage sans pression : une variante plus douce reste une vraie séance.':style==='family'?'Je fais simple, drôle et vivant pour que tout le monde puisse jouer.':'';
    const arrival=feeling==='mixed'?'Je sais que les énergies sont différentes : je donnerai deux variantes à chaque bloc.':feeling==='restart'?'On repart progressivement et je te demande régulièrement comment ça va.':'';
    return pickVoiceLine([
      `Bienvenue dans votre séance ! ${personal} ${arrival} ${continuity?continuity+' ':''}${animatePersonalTouch(session)}`,
      `C’est moi qui anime, tu n’as qu’à bouger et me répondre ! ${personal} ${arrival} ${animatePersonalTouch(session)}`,
      `Le Club Dolcia est ouvert ! ${personal} ${arrival} ${animatePersonalTouch(session)}`
    ]);
  }
  ...
  if(!last&&session.index===midpoint&&session.index>0)return pickVoiceLine([
    'Petite pause d’une seconde : dis-moi honnêtement, ça te plaît toujours autant ?',
    'On fait un point vite fait — tout le monde se sent bien, on continue comme ça ?',
    'Juste pour être sûr : ce rythme te va, ou on ajuste quelque chose ?'
  ]);
  ...
  if(last==='livelier')return pickVoiceLine([
    'Ah, tu en veux plus ?! On réveille tout ça, allez !!',
    'Parfait, exactement ce qu’il fallait dire !! La suite va bouger, accroche-toi !',
    'On monte le son !! La prochaine mission va être costaud, tout le monde !'
  ]);
  if(last==='skip')return pickVoiceLine([
    'Pas de souci, celle-là n’était pas pour toi — on efface et on avance, aucun problème !',
    'Ok, on passe ! Ça arrive, la suite va te plaire davantage, j’en suis sûr.',
    'Aucun souci, on l’oublie complètement. On continue sur notre lancée !'
  ]);
  return pickVoiceLine([
    'Allez, on continue sur cette belle énergie !! J’adapte la suite pour toi.',
...
function animateStepText(session){
  ...
  const hype=pickVoiceLine(finalStep?[
    ...
    'C’est le moment, lance-toi !'
  ]:[
    'Go, à toi de jouer !',
    'Allez, c’est ton moment !',
    'On garde cette énergie, en piste !',
    'Et maintenant, place à toi !'
  ]);
  if(last==='calmer')return`${hype} ${base} Fais-le tranquillement, à ton rythme, sans chrono ni compétition.`;
...
function animatePhaseLabel(phase){return phase==='echauffement'?'Échauffement':phase==='coeur'?'Cœur de séance':phase==='retour'?'Retour au calme':''}
...
function renderDolciaAnimateLive(options={}){
  ...
  document.body.insertAdjacentHTML('beforeend',`<div class="modal animate-live theme-${energyTheme.id}" id="animateLive"><article>
    ...
    <div class="animate-response"><small>Comment vit le groupe ?</small><div>
      ...
      ${session.item.coach?'<button class="safety-reaction" onclick="reactDolciaAnimate(\'pain\')">Douleur / on arrête</button>':''}
    </div></div>
...
// Moteur de comportements d'animateur — sépare CE QUI se joue (les 20 expériences) de COMMENT D
// l'anime, pour multiplier la variété sans écrire des centaines de scripts. Chaque comportement
// n'est déclenché que par un signal réellement disponible (temps écoulé, réaction explicite,
// score, fin de session) — jamais une perception que Dolcia n'a pas (elle ne voit ni n'entend
// personne en dehors des taps et, en mode vocal, des mots réellement prononcés).
const ANIMATOR_BEHAVIORS={
  feliciter_sans_gagnant:[ // signal réel : fin de session avec deux équipes
    (a,b)=>`${a} et ${b} ont toutes les deux été formidables — l'énergie était là des deux côtés, c'est bien ça qui compte.`,
    (a,b)=>`Franchement, difficile de départager ${a} et ${b} aujourd'hui — et c'est tant mieux.`,
    (a,b)=>`${a}, ${b} — tout le monde a donné le meilleur des deux côtés. C'est ça, la vraie victoire.`
  ],
  relance_apres_silence:[ // signal réel : lastInteractionAt dépasse un vrai délai
    ()=>'toujours là ? Je te laisse le temps qu\'il faut, dis-moi quand tu es prêt.',
    ()=>'pas de pression — on reprend dès que tu veux.',
    ()=>'je patiente avec toi. Un mot, et on continue.'
  ],
  conclure_avec_emotion:[ // signal réel : session réellement terminée (completedAt existe)
    ()=>'C\'est déjà fini — et c\'était vraiment bien.',
    ()=>'Voilà, ce moment restera. Merci de l\'avoir vécu avec moi.',
    ()=>'On s\'arrête là, sur une bonne note. À la prochaine fois.'
  ],
  suspense_avant_reveal:[ // signal réel : une révélation va suivre (blind test, quiz, chasse au trésor)
...
function pickAnimatorLine(behavior,...args){
  const pool=ANIMATOR_BEHAVIORS[behavior];if(!pool)return'';
  const fn=pool[Math.floor(Math.random()*pool.length)];
  return fn(...args);
}
function animateNudgeLine(session){
  const captain=animateCaptain(session);
  if((session.reactions||[]).length===0){const line=pickAnimatorLine('relance_apres_silence');return`${captain}, ${line.charAt(0).toLowerCase()}${line.slice(1)}`}
  if((session.laughs||0)>0)return`Je note que le rire est bien là. On garde cette énergie ou je te prépare un virage inattendu ?`;
  if((session.streak||0)>=2)return`Belle série. Je peux monter d’un cran, mais seulement si tu en as envie.`;
  return`Petit point d’étape : l’énergie te convient, ou j’ajuste la suite ?`
}
...
function reactDolciaAnimate(reaction){
  const session=state.activeAnimate;if(!session)return;
  if(reaction==='pain'){
    session.reactions.push(reaction);session.adjustments.push({step:session.index,reaction,at:new Date().toISOString()});session.status='paused';save();
    stopDolciaTheme();stopAnimatePulse();window.clearTimeout(animateAutoAdvanceTimer);window.clearTimeout(animateNudgeTimer);
    const message='On arrête maintenant. Asseyez-vous ou installez-vous confortablement. En cas de douleur importante, malaise, douleur thoracique ou difficulté à respirer, demandez une aide médicale.';
    session.messages.push({role:'assistant',content:message});setDVisualState('calm');renderDolciaAnimateLive({speak:false});playPremiumVoice(message);return
  }
...
function completeDolciaAnimateStep(){
  ...
  session.elan+=10;
  if(session.teams){session.teams[session.teams.turn].score+=10;session.teams.turn=session.teams.turn==='a'?'b':'a'}
  celebrateAnimateMoment();
  if(session.index>=session.item.steps.length-1)return finishDolciaAnimate();
  session.index+=1;session.captainIndex=(session.captainIndex+1)%(session.roster?.length||1);save();renderDolciaAnimateLive();pushAnimateSessionState()
}
...
// Le niveau de richesse détermine uniquement CE QUI s'affiche, jamais SI la fiche s'affiche.
// Une fiche existe toujours — c'est la promesse centrale de cette architecture.
function detailQuality(item,verified){
  if(verified&&item.address&&(item.hours?.length||item.isOpen!=null))return'rich';
  if(item.address||item.date)return'standard';
  return'minimal';
}
// Les actions sont des données, jamais du code par bouton. Chaque action ne peut apparaître que
// si la donnée réelle qui la justifie existe (evidence) — jamais une action fabriquée pour remplir
// l'écran. Une seule action principale (priority 1), toujours interne à Dolcia tant qu'aucune
// réservation directe réelle n'existe — jamais simulée.
function detailActions(item){
  const actions=[
    {type:'add_agenda',label:'Ajouter à mon agenda',priority:1,handler:`addAgenda('${item.id}')`,isExternal:false,evidence:'action interne toujours disponible'},
    {type:'save_idea',label:'Garder dans mes envies',priority:2,handler:`saveDetailIdea('${item.id}')`,isExternal:false,evidence:'action interne toujours disponible'}
  ];
  actions.push({type:'map',label:'Voir sur la carte',priority:10,target:item.googleUrl||detailMapUrl(item),isExternal:true,evidence:item.address?'adresse vérifiée':'recherche par nom et destination, adresse non confirmée'});
  if(item.phone)actions.push({type:'call',label:'Appeler',priority:11,target:`tel:${item.phone.replace(/\s+/g,'')}`,isExternal:true,evidence:'téléphone vérifié'});
  if(item.website)actions.push({type:'website',label:'Site officiel',priority:12,target:item.website,isExternal:true,evidence:'site officiel vérifié'});
  if(item.booking)actions.push({type:'booking',label:'Billetterie',priority:13,target:item.booking,isExternal:true,evidence:'lien de réservation externe fourni'});
  return actions.sort((a,b)=>a.priority-b.priority);
}
function renderDetailActions(actions){
  return actions.map(a=>{
    const cls=a.priority===1?'primary':'secondary';
    if(a.handler)return`<button class="${cls}" onclick="${a.handler}">${esc(a.label)}</button>`;
    const isTel=a.target.startsWith('tel:');
    return`<button class="${cls}${isTel?' phone-cta':''}" onclick="${isTel?`window.location.href='${a.target}'`:`window.open('${esc(a.target)}','_blank','noopener,noreferrer')`}" aria-label="${esc(a.label)}${a.isExternal?' (lien externe, sécurisé)':''}">${esc(a.label)}${a.isExternal?' ↗':''}</button>`;
  }).join('');
}
let detailScrollMemory=null;
async function openDetail(id){
  detailScrollMemory=window.scrollY;
  ...
  const facts=[
    item.address?`<div><b>Adresse</b><span>${esc(item.address)}</span></div>`:'<div><b>Adresse</b><span>À confirmer</span></div>',
    `<div><b>Durée</b><span class="duration-${activityDurationPresentation(item).status}">${esc(activityDurationPresentation(item).label)}</span></div>`,
    item.date?`<div><b>Date</b><span>${new Date(item.date).toLocaleString('fr-FR',{dateStyle:'long',timeStyle:'short'})}</span></div>`:(item.timeKnown===false?'<div><b>Horaire</b><span>À confirmer</span></div>':''),
    item.free?'<div><b>Prix</b><span>Gratuit confirmé</span></div>':item.price!=null?`<div><b>Prix</b><span>${'€'.repeat(Math.min(item.price,4))}</span></div>`:(item.priceKnown===false?'<div><b>Prix</b><span>À confirmer</span></div>':''),
    item.phone?`<div><b>Téléphone</b><span><a href="tel:${esc(item.phone.replace(/\s+/g,''))}" class="phone-link">${esc(item.phone)}</a></span></div>`:'',
...
function closeDetail(){
  ...
  if(detailScrollMemory!=null){window.scrollTo(0,detailScrollMemory);detailScrollMemory=null}
}
function refreshNavBadge(){
  const agendaButton=[...document.querySelectorAll('.bottom-nav .nav-item')].find(button=>plainText(button.textContent||'').startsWith('agenda'));
  if(agendaButton)agendaButton.innerHTML=`<span>▤</span>Agenda${state.agenda.length?` · ${state.agenda.length}`:''}`;
}
let addConfirmTimer=null;
function showAddConfirmation(message='Ajouté à votre programme'){
  document.querySelector('#addConfirm')?.remove();
  document.body.insertAdjacentHTML('beforeend',`<div class="add-confirm" id="addConfirm" role="status"><span>${esc(message)}</span><div><button onclick="document.querySelector('#addConfirm')?.remove()">Continuer à explorer</button><button class="primary" onclick="renderAgenda()">Voir mon agenda</button></div></div>`);
...
async function addAgenda(id,slotLabel=''){
  ...
  if((compatibility==='open-not-session'||compatibility==='unknown')&&requiresPublishedSession(i))showToast('Réservation ou créneau à confirmer directement auprès de l’établissement');
...
window.shareAnimateSession=shareAnimateSession;window.openAnimateSharePicker=openAnimateSharePicker;window.stopShareAnimateSession=stopShareAnimateSession;window.shareAnimateLink=shareAnimateLink;
window.openAnimateJoin=openAnimateJoin;window.joinAnimateSession=joinAnimateSession;window.leaveAnimateFollow=leaveAnimateFollow;
// Mode suiveur : quelqu'un rejoint une session partagée via un lien scanné (QR) ou un code saisi
// à la main. Vue en lecture seule qui se met à jour toute seule — jamais de contrôle sur l'avancement,
// seule la personne qui a créé le code garde la main.
let animateFollowTimer=null;
function stopAnimateFollow(){window.clearInterval(animateFollowTimer);animateFollowTimer=null}
function openAnimateJoin(prefillCode){
  document.querySelector('#animateJoin')?.remove();
  document.body.insertAdjacentHTML('beforeend',`<div class="modal" id="animateJoin"><article class="animate-join-card"><button class="close" onclick="document.querySelector('#animateJoin')?.remove()">×</button><h3>Rejoindre une session Dolcia Anime</h3><p>Demandez le code à la personne qui anime — 6 caractères, sans espace.</p><input id="animateJoinCode" maxlength="6" placeholder="EX. 6JZ337" value="${esc(prefillCode||'')}" style="text-transform:uppercase"><button class="primary" onclick="joinAnimateSession()">Rejoindre</button></article></div>`);
  if(prefillCode)joinAnimateSession();
}
async function joinAnimateSession(){
  const input=document.querySelector('#animateJoinCode'),code=(input?.value||'').toUpperCase().trim();
  if(code.length<4)return showToast('Entrez le code complet, à 6 caractères.');
  document.querySelector('#animateJoin')?.remove();
  state.followingAnimateCode=code;
  renderAnimateFollowerView();
  await refreshAnimateFollowerState();
  stopAnimateFollow();
  animateFollowTimer=window.setInterval(refreshAnimateFollowerState,4000);
}
async function refreshAnimateFollowerState(){
  const code=state.followingAnimateCode;if(!code)return;
  try{
    const response=await fetch(`/api/events?service=animate-session&code=${encodeURIComponent(code)}`);
    const data=await response.json().catch(()=>({}));
    if(!response.ok){
      stopAnimateFollow();
      document.querySelector('#animateFollowShell')?.insertAdjacentHTML('afterbegin',`<p class="animate-follow-error">${response.status===410?'Ce code a expiré.':'Session introuvable.'}</p>`);
      return;
    }
    state.followingAnimateState=data.state;
    renderAnimateFollowerView();
  }catch{}
}
function leaveAnimateFollow(){stopAnimateFollow();state.followingAnimateCode=null;state.followingAnimateState=null;document.querySelector('#animateFollowShell')?.remove()}
function renderAnimateFollowerView(){
  document.querySelector('#animateFollowShell')?.remove();
  const live=state.followingAnimateState;
  const step=live?.steps?.[live.index];
  document.body.insertAdjacentHTML('beforeend',`<div class="modal animate-live theme-${live?.energyTheme||'signature'}" id="animateFollowShell"><article>
    <div class="animate-energy-scene" aria-hidden="true"><span></span><span></span><span></span><span></span></div>
    <button class="close" onclick="leaveAnimateFollow()" aria-label="Quitter le suivi">×</button>
    <header class="animate-presence">${dMascotMark('large')}<div><small>Vous suivez ${live?live.captain+' · ':''}en direct</small><strong>${esc(live?.name||'En attente…')}</strong></div></header>
    ${live?`<div class="animate-session-meta"><span>Mission ${(live.index||0)+1}/${live.steps.length}</span><span>${esc(live.captain||'')}</span></div>
    <h2>${esc(live.name)}</h2>
    <div class="animate-live-step"><time>${esc(step?.[0]||'')}</time><p>${esc(step?.[1]||'')}</p></div>
    ${live.teams?`<div class="animate-teams"><div><b>${live.teams.a.score}</b><span>${esc(live.teams.a.name)}</span></div><div class="animate-teams-vs">vs</div><div><b>${live.teams.b.score}</b><span>${esc(live.teams.b.name)}</span></div></div>`:''}
    <p class="animate-follow-note">Vous suivez en direct — la personne qui anime garde la main sur l'avancement.</p>`:`<p class="animate-follow-note">En attente que la session démarre…</p>`}
  </article></div>`);
}
const urlParams=new URLSearchParams(window.location.search);
if(urlParams.get('join'))openAnimateJoin(urlParams.get('join'));
home();
