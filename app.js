...
function renderDate(){
  app.innerHTML=shell(`<section class="composer"><div class="composer-shell"><div class="composer-head"><div><span class="kicker">Le moment</span><h2>Où et quand voulez-vous sortir ?</h2></div><div class="step-count">Point de départ<div class="progress"><span style="width:12%"></span></div></div></div><p class="step-lead">Choisissez une période une seule fois. « À partir de maintenant » couvre automatiquement le reste de l’après-midi et la soirée. Dolcia ne vous redemandera pas la durée.</p><div class="destination-row">${Object.entries(DESTINATIONS).map(([id,d])=>`<button class="destination-pill ${state.location.name===d.name?'active':''}" onclick="setDestination('${id}')">${d.name}</button>`).join('')}<button class="destination-pill" onclick="useLocation()">Autour de moi</button></div><div class="date-grid"><div class="date-presets">${[['now','À partir de maintenant',`Dès ${new Date().toLocaleTimeString('fr-FR',{hour:'2-digit',minute:'2-digit'})}, après-midi + soirée`],['tonight','Ce soir','À partir de 18 heures'],['tomorrow','Demain','Choisissez ensuite votre créneau'],['weekend','Ce week-end','Du samedi au dimanche']].map(x=>`<button class="date-pill ${state.dateMode===x[0]?'active':''}" onclick="setDate('${x[0]}')"><b>${x[1]}</b><br><small>${x[2]}</small></button>`).join('')}</div>${calendar()}</div><div class="composer-actions"><button class="secondary" onclick="home()">← Accueil</button><button class="primary" onclick="confirmDate()">Continuer →</button></div></div></section>`,'compose');
}
function confirmDate(){if(tripDays()>1||state.dateMode==='weekend'){state.answers.duration='stay';state.step=1}else if(state.dateMode==='tonight'){state.answers.duration='evening';state.step=1}else if(state.dateMode==='now'){state.answers.duration='afternoon_evening';state.step=1}else state.step=0;renderComposer()}
...
function setDate(mode){const now=new Date();state.dateMode=mode;let d=new Date(now);if(mode==='tonight')d.setHours(Math.max(18,now.getHours()),now.getMinutes(),0,0);if(mode==='tomorrow'){d.setDate(d.getDate()+1);d.setHours(9,0,0,0)}if(mode==='weekend'){const add=(6-d.getDay()+7)%7;d.setDate(d.getDate()+add);d.setHours(9,0,0,0)}state.dateStart=d;state.dateEnd=mode==='weekend'?new Date(d.getFullYear(),d.getMonth(),d.getDate()+1,23,59):new Date(d.getFullYear(),d.getMonth(),d.getDate(),23,59);state.month=new Date(d.getFullYear(),d.getMonth(),1);renderDate()}
