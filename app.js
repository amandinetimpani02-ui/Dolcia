...
function mountHomeConcierge(){const hero=document.querySelector('.hero');if(!hero)return;hero.insertAdjacentHTML('afterend',`<section class="eclat-concierge"><div class="eclat-visual"><img src="/assets/dolcia-eclat-concept.png" alt="Le D vivant et L’Éclat Dolcia"></div><div class="eclat-copy"><span class="kicker">Le Concierge vivant</span><h2>Vous ne savez pas quoi faire ?<br><em>Dolcia, si.</em></h2><p>Parlez librement, même sans idée précise. L’Éclat pose uniquement la question décisive, comprend votre moment et transforme l’immense catalogue en une expérience qui vous ressemble.</p><div class="eclat-actions"><button class="primary" onclick="openEclatBrief(false)">Écrire à Dolcia</button><button class="secondary" onclick="openEclatBrief(true)">Parler à Dolcia</button><button class="ghost" onclick="startCompose()">Je ne sais vraiment pas</button></div><small>Vos réponses orientent les propositions sans masquer le reste du catalogue.</small></div></section>`)}
function openEclatBrief(voice){state.catalogFilters=state.catalogFilters||{family:'all',sort:'recommended',availability:'all',query:'',limit:60,preferred:[]};if(!state.allItems.length){startCompose();showToast('Dolcia commence par votre disponibilité, puis prendra le relais');return}renderResults();if(voice)setTimeout(startVoiceBrief,150);else setTimeout(()=>document.querySelector('#momentBriefInput')?.focus(),150)}
...
async function compose(){
  ...
  try{
    ...
    const broadcastVenues=state.majorChoice?`/api/broadcast-venues?event=${encodeURIComponent(state.majorChoice)}&lat=${state.location.lat}&lng=${state.location.lng}&radius=${Math.round(state.radius/1000)}`:null;
  ...
  state.majorChoice?renderSurprise():renderResults();
}
...
function scoreItems(items){
  ...
  return items.filter(i=>{
    ...
    return {...i,score,distance,quality:qualityLevel(i)};
...
function renderResults(){
  ...
  const filters=state.catalogFilters;filters.preferred=filters.preferred||[];const items=catalogSelection(),shown=items.slice(0,filters.limit),families=[['all','Tout'],['fun','Fun & famille'],['nature','Nature & mer'],['culture','Culture & événements'],['food','Restaurants & gourmandises'],['wellness','Bien-être'],['night','Sortir le soir'],['hotel','Hébergements']];
  const counts=Object.fromEntries(families.map(([id])=>[id,id==='all'?state.allItems.length:state.allItems.filter(item=>catalogFamily(item)===id).length]));
  setTimeout(()=>{mountCatalogPreferences(families);mountVoiceBrief();mountGuidedCompass()},0);
  app.innerHTML=shell(`<section class="catalog-hero"><span class="kicker">Explorer Dolcia</span><h1>Tout ce que vous pouvez vivre.</h1><p>${items.length} propositions dans votre sélection. Les filtres vous aident à choisir ; ils ne réduisent jamais le catalogue en secret.</p><button class="surprise-hero" onclick="surprise()"><span>Vous préférez que Dolcia choisisse ?</span><strong>${composeCta()}</strong><b>→</b></button></section><section class="catalog-shell"><div class="catalog-toolbar"><div class="catalog-search"><input id="catalogSearch" value="${esc(filters.query)}" placeholder="Une envie, un lieu, une activité…"><button onclick="applyCatalogSearch()">Rechercher</button></div><div class="catalog-families">${families.filter(([id])=>id!=='hotel'||state.answers.duration==='stay').map(([id,label])=>`<button class="${filters.family===id?'selected':''}" onclick="setCatalogFamily('${id}')">${label}<small>${counts[id]}</small></button>`).join('')}</div><div class="catalog-controls"><select onchange="setCatalogSort(this.value)"><option value="recommended" ${filters.sort==='recommended'?'selected':''}>Recommandés pour moi</option><option value="distance" ${filters.sort==='distance'?'selected':''}>Les plus proches</option><option value="rating" ${filters.sort==='rating'?'selected':''}>Les mieux notés</option><option value="new" ${filters.sort==='new'?'selected':''}>Événements en premier</option></select><select onchange="setCatalogAvailability(this.value)"><option value="all" ${filters.availability==='all'?'selected':''}>Toutes les disponibilités</option><option value="open" ${filters.availability==='open'?'selected':''}>Ouverts maintenant</option><option value="scheduled" ${filters.availability==='scheduled'?'selected':''}>Événements avec horaire</option></select><button onclick="startCompose()">Modifier date, budget et envies</button></div></div><div class="catalog-summary"><strong>${items.length} idées</strong><span>${shown.length<items.length?`${shown.length} affichées · chargez la suite quand vous le souhaitez`:'Catalogue affiché en totalité'}</span></div><div class="catalog-list">${shown.map((item,index)=>experience(item,index,'')).join('')}</div>${shown.length<items.length?`<button class="catalog-more" onclick="showMoreCatalog()">Afficher ${Math.min(60,items.length-shown.length)} propositions supplémentaires</button>`:''}</section>`,'discover')
}
...
function renderSurprise(){
  ...
  app.innerHTML=shell(`<section class="surprise-cover"><div class="surprise-cover-photo"></div><div class="surprise-cover-copy"><span class="kicker">Votre programme composé</span><h1>Votre moment.<br><em>Aux petits oignons.</em></h1><p>Dolcia a choisi et ordonné chaque étape selon la durée, la date, la météo, votre budget et ce qu’elle apprend de vos goûts.</p><div class="tags">${criteria.map(x=>`<span class="tag">${esc(x)}</span>`).join('')}</div><div class="surprise-controls"><button class="primary" onclick="adoptSurprise()">Ajouter ce programme à mon agenda</button><button class="secondary" onclick="surprise(true)">Tout recomposer ↻</button><button class="secondary" onclick="renderResults()">Voir toutes les propositions</button></div></div></section><section class="program surprise-program"><div class="surprise-note"><strong>Un vrai programme, pas une liste.</strong><span>Chaque activité correspond à un créneau. Changez une seule idée, précisez votre envie ou adoptez le programme composé.</span></div>${broadcastProgramPanel()}${state.answers.duration==='stay'?lodgingPreference():''}${state.program.length?state.program.map(programSlot).join(''):emptyState()}<div class="agenda-guidance"><strong>Votre programme est prêt.</strong><span>Adoptez-le en entier ou ajoutez seulement certains moments.</span><button onclick="renderAgenda()">Voir mon agenda →</button></div></section>`,'discover');
}
function mountProgramIntelligence(){const anchor=document.querySelector('.surprise-note');if(!anchor)return;const discovery=state.program.some(slot=>slot.item.discoveryPick),official=state.program.filter(slot=>slot.item.official&&slot.item.date).length,verified=state.program.filter(slot=>slot.item.quality==='verified'||slot.item.official).length;anchor.insertAdjacentHTML('afterend',`<aside class="program-intelligence"><div><span>Signature Dolcia</span><strong>Pourquoi ce programme vous ressemble</strong></div><div class="intelligence-signals"><span><b>${verified}</b> choix solidement documentés</span><span><b>${official}</b> rendez-vous officiels à vos dates</span><span><b>${discovery?'1':'—'}</b> ${discovery?'pépite inattendue mais cohérente':'surprise ajoutée seulement si elle est assez pertinente'}</span><span><b>100 %</b> ordonné selon votre rythme</span></div></aside>${state.answers.who==='family'?familyRhythmPanel():''}`)}
...
function why(i){const temp=state.weather?.main?.temp??18,vibes=state.answers.vibes||[];if(i.discoveryPick)return'La pépite que vous n’auriez peut-être pas cherchée';if(i.official&&i.date)return'Événement officiel à votre date';if(temp>=25&&/plage|nautique|voile|surf|paddle|kayak|piscine/.test(plainText(i.name||'')))return'Idéal avec la météo du moment';if((state.tasteProfile[i.category]||0)>1)return'Inspiré de ce que vous aimez déjà';if(matchesIntentions(i,vibes))return'Correspond à votre envie maintenant';if(i.rating>=4.6&&i.reviews>=100)return'Valeur sûre très appréciée';if(i.distance!=null&&i.distance<2)return'Belle idée tout près de vous';return'Sélection Dolcia pour élargir vos envies'}
...
async function openDetail(id){
  ...
  const facts=[
    item.address?`<div><b>Adresse</b><span>${esc(item.address)}</span></div>`:'<div><b>Adresse</b><span>Lieu exact à confirmer</span></div>',
    item.date?`<div><b>Date</b><span>${new Date(item.date).toLocaleString('fr-FR',{dateStyle:'long',timeStyle:'short'})}</span></div>`:'',
    item.phone?`<div><b>Téléphone</b><span>${esc(item.phone)}</span></div>`:'<div><b>Téléphone</b><span>Non communiqué</span></div>',
...
function postExperiencePanel(item){const feelings=[['unforgettable','Inoubliable'],['loved','J’ai adoré'],['nice','Un joli moment'],['notforme','Pas pour moi'],['disappointing','Décevant']];const tags=[['value','Bon rapport qualité-prix'],['family','Parfait avec les enfants'],['couple','Idéal à deux'],['discovery','Belle découverte'],['welcome','Accueil remarquable'],['again','À refaire'],['expensive','Trop cher'],['far','Trop loin'],['touristy','Trop touristique']];const selected=state.experienceTags[item.id]||[];return `<div class="experience-memory"><span class="emotion-kicker">Après l’expérience</span><h3>Comment vous êtes-vous senti après ce moment ?</h3><div class="feeling-scale">${feelings.map(([id,label])=>`<button class="${state.experienceFeelings[item.id]===id?'selected':''}" onclick="saveFeeling('${item.id}','${id}')">${label}</button>`).join('')}</div><p>Qu’est-ce qui a compté ?</p><div class="memory-tags">${tags.map(([id,label])=>`<button class="${selected.includes(id)?'selected':''}" onclick="toggleExperienceTag('${item.id}','${id}')">${label}</button>`).join('')}</div><small>Votre ressenti personnel nourrit vos prochaines compositions. Il ne remplace jamais les avis Google.</small></div>`}
...
function renderAgenda(){state.view='agenda';state.agenda.sort((a,b)=>new Date(a.agendaDate)-new Date(b.agendaDate));const cards=state.agenda.map((item,index)=>{const date=new Date(item.agendaDate||state.dateStart),day=iso(date),time=`${String(date.getHours()).padStart(2,'0')}:${String(date.getMinutes()).padStart(2,'0')}`;return `<article class="agenda-card agenda-editable ${item.fromSurprise?'from-surprise':''}"><div class="agenda-time"><input type="time" value="${time}" onchange="updateAgendaTime('${item.id}',this.value)"><input type="date" value="${day}" onchange="updateAgendaDate('${item.id}',this.value)"></div><img src="${itemImage(item)}" alt=""><div><span class="agenda-slot">${esc(item.agendaSlot||'Moment choisi')}</span><h3>${esc(item.name)}</h3><p>${esc(item.address||state.location.name)}</p><div class="agenda-tools"><button onclick="moveAgenda(${index},-1)" ${index===0?'disabled':''}>↑ Plus tôt</button><button onclick="moveAgenda(${index},1)" ${index===state.agenda.length-1?'disabled':''}>↓ Plus tard</button><button onclick="openDetail('${item.id}')">Voir la fiche</button><button onclick="removeAgenda('${item.id}')">Retirer</button></div></div></article>`}).join('');app.innerHTML=shell(`<section class="agenda-view agenda-deluxe"><div class="agenda-title"><div><span class="kicker">Votre temps, orchestré</span><h2>Mon programme.</h2><p>Modifiez directement la date et l’heure de chaque moment.</p></div><button class="primary" onclick="openComposition()">${state.program.length?'Revoir mon programme composé':'Compose-moi mon programme'}</button></div>${cards||`<div class="empty-state"><h3>Votre programme commence ici.</h3><p>Dolcia construira un véritable agenda, heure par heure.</p><button class="primary" onclick="startCompose()">Compose-moi mon programme</button></div>`}</section>`,'agenda')}
